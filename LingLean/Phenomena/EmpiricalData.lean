...
A syntactic theory makes predictions by providing "linking functions"
...
The Rational Speech Act framework provides a principled linking function
...
/-- Types of empirical measures -/
inductive MeasureType where
  | categorical     -- binary: grammatical vs ungrammatical
  | graded          -- ordinal scale (e.g., 1-7 Likert)
  | continuous      -- continuous measure (reading time, surprisal)
  | proportion      -- probability/proportion (0-1)
...
-- ============================================================================
-- RSA-Style Linking Functions
-- ============================================================================

/-
The Rational Speech Act (RSA) framework provides linking functions:

1. Literal Listener (L0): P(meaning | utterance) ∝ ⟦utterance⟧(meaning)
2. Pragmatic Speaker (S1): P(utterance | meaning) ∝ exp(α · log P_L0(meaning | utterance) - cost(utterance))
3. Pragmatic Listener (L1): P(meaning | utterance) ∝ P_S1(utterance | meaning) · P(meaning)

These can be composed to derive pragmatic interpretations from literal semantics.
-/
...
structure RSAParams where
  ...
  alpha : Float := 1.0
  /-- Utterance cost function weight -/
  costWeight : Float := 0.0
  /-- Prior over meanings -/
  meaningPrior : List (String × Float) := []
  deriving Repr

/-- Predicted choice probability from RSA model -/
structure RSAPrediction where
  /-- Predicted speaker choice distribution -/
  speakerChoices : List (List Word × Float)
  /-- Predicted listener interpretation distribution -/
  listenerInterpretations : List (String × Float)
  deriving Repr

/-- A pragmatic linking function maps semantics + context to choice probabilities -/
class PragmaticLinking (Semantics Context : Type) where
  /-- Literal interpretation -/
  literalInterpret : Semantics → Context → List String → List (String × Float)
  /-- Pragmatic speaker model -/
  speakerModel : Semantics → Context → String → RSAParams → List (List Word × Float)
  /-- Pragmatic listener model -/
  listenerModel : Semantics → Context → List Word → RSAParams → List (String × Float)
...
-- ============================================================================
-- Connecting Syntax to Pragmatics
-- ============================================================================

/-
The full pipeline:
  Syntax → Semantics → Pragmatics → Behavior

1. Syntactic theory provides derivations
2. Semantic interpretation maps derivations to meanings
3. Pragmatic reasoning (RSA) maps meanings + context to predictions
