...
def checkCatMatch (t : CoordTree) : Bool :=
  t.coordinations.all fun coord =>
    match t.words.get? coord.firstConjunct with
    | some firstWord =>
      coord.otherConjuncts.all fun idx =>
        match t.words.get? idx with
...
def coordCategory (t : CoordTree) (coord : CoordStr) : Option Cat :=
  t.words.get? coord.firstConjunct |>.map (·.cat)
...
theorem johnAndMary_cat_match :
    checkCatMatch ex_johnAndMarySleep = true := rfl

/-- VP coordination has matching argument structures -/
theorem seesAndHears_argstr_match :
    checkArgStrMatch ex_johnSeesAndHearsMary = true := rfl
