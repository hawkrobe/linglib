...
open Degree Quantification Set
...
theorem forall_collapse [Finite Entity] {girl : Entity → Prop} (hg : ∃ x, girl x)
  ...
  highScope_maxIn_iff_lowScope (monotone_every_sem girl) (λ h => let ⟨x, hx⟩ := hg; h x hx) hU
...
theorem exists_collapse [Finite Entity] (girl : Entity → Prop) (μ : Entity → D) {U : Set D}
  ...
  highScope_maxIn_iff_lowScope (monotone_some_sem girl) (λ ⟨_, _, h⟩ => h) hU
...
theorem exactly_high_atLeast [Fintype Entity] {girl : Entity → Prop} {μ : Entity → D} {m : D}
  ...
  rw [exactly_eq_meet_at_least_at_most] at h
  exact isGreatest_scopeDegrees_of_inf (antitone_at_most_n_sem 2 girl) h
...
theorem negRaising_collapse [Finite W] [DenselyOrdered D] {Des : Set W} (hDes : Des.Nonempty)
    ...
    (∃ m ∈ Iio t, IsGLB (scopeDegrees (no_sem (· ∈ Des)) ℓ) m) ↔
      lowScope (maxIn (Iio t)) (every_sem (· ∈ Des)) ℓ := by
  have hex : ∃ m, IsGreatest (scopeDegrees (some_sem (· ∈ Des)) ℓ) m :=
    let ⟨w, hw⟩ := exists_isGreatest_scopeDegrees (monotone_some_sem _) (λ ⟨_, _, h⟩ => h)
      (let ⟨w, hw⟩ := hDes; ⟨ℓ w, w, hw, le_rfl⟩)
    ⟨_, hw⟩
  rw [scopeDegrees_no, lowScope_maxIn]
  refine (exists_congr λ m => and_congr_right λ _ =>
    isGLB_compl_scopeDegrees_iff (monotone_some_sem _) hex).trans
...
theorem est_iff_absoluteSuperlative [Finite Entity] {μ : Entity → D} {C : Set Entity}
  ...
  refine (⟨λ ⟨δ, hδ, _, rfl, hlt⟩ => ⟨δ, hlt, hδ⟩, λ ⟨δ, hlt, hδ⟩ => ⟨δ, hδ, x, rfl, hlt⟩⟩ :
    est μ C x ↔ highScope (maxIn (Iio (μ x))) (some_sem λ y => y ∈ C ∧ y ≠ x) μ).trans
