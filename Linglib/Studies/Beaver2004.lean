...
open Discourse.Centering Core.Optimization.OT Core.Optimization.Evaluation
...
structure Candidate (E : Type) (R : Type) where
  ...
  /-- DISJOINT: co-arguments of any predicate are distinct entities?
      ...
      `Linglib/Semantics/ArgumentStructure/Defs.lean` provides
...
/-- **OT lex-min witness on Beaver (12)**: under the canonical COT
    ...
    `Core.Optimization.OT` substrate's `mkProfile` /
