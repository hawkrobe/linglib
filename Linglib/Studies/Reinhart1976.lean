...
The concrete c-command facts (§5-§7) use `Branching.kCommandAt` — the
...
open Core.Order
open Core.Order.Branching (isBranchingAt kCommandAt yield)
open Syntax (Tree)
...
/-! ### C-command as `kCommandAt` over concrete trees

The concrete tests use the [barker-pullum-1990] tower:
[reinhart-1976]'s c-command is `kCommandAt t` — the B&P command relation
...
([reinhart-1976]'s c-command = B&P K-command). -/
...
/-- The subject `⟨[0]⟩` k-commands the object `⟨[1,1]⟩`: the only
    ...
    everything. -/
theorem subject_kcommands_object :
    (⟨[0]⟩, ⟨[1, 1]⟩) ∈ kCommandAt johnSawMary := by decide
...
theorem object_not_kcommands_subject :
    (⟨[1, 1]⟩, ⟨[0]⟩) ∉ kCommandAt johnSawMary := by decide

/-- The subject c-commands into the VP (here, the verb `⟨[1,0]⟩`). -/
theorem subject_kcommands_into_vp :
    (⟨[0]⟩, ⟨[1, 0]⟩) ∈ kCommandAt johnSawMary := by decide

/-- Object and verb mutually k-command (co-arguments inside the VP). -/
theorem object_verb_mutual_kcommand :
    (⟨[1, 1]⟩, ⟨[1, 0]⟩) ∈ kCommandAt johnSawMary ∧
    (⟨[1, 0]⟩, ⟨[1, 1]⟩) ∈ kCommandAt johnSawMary := by
  constructor <;> decide

/-- C-command is not symmetric: subject k-commands object, not conversely. -/
theorem kcommand_asymmetric_example :
    (⟨[0]⟩, ⟨[1, 1]⟩) ∈ kCommandAt johnSawMary ∧
    (⟨[1, 1]⟩, ⟨[0]⟩) ∉ kCommandAt johnSawMary :=
  ⟨subject_kcommands_object, object_not_kcommands_subject⟩
...
theorem preposed_pp_no_kcommand :
    (⟨[0, 1]⟩, ⟨[1, 0]⟩) ∉ kCommandAt nearDanHeSaw := by decide
...
theorem matrix_subject_kcommands_embedded :
    (⟨[0]⟩, ⟨[1, 1, 0]⟩) ∈ kCommandAt complexSubject := by decide
...
theorem deep_pronoun_not_kcommands_embedded :
    (⟨[0, 1, 1, 1, 1]⟩, ⟨[1, 1, 0]⟩) ∉ kCommandAt complexSubject := by decide
...
def corefPermittedAt (t : Tree Unit String) (isPronA isPronB : Prop)
  ...
  ((a, b) ∈ kCommandAt t → isPronB) ∧ ((b, a) ∈ kCommandAt t → isPronA)
...
private theorem matrix_kcommands_embedded :
    (⟨[0]⟩, ⟨[1, 1, 0]⟩) ∈ kCommandAt deniedParadigm ∧
    (⟨[1, 1, 0]⟩, ⟨[0]⟩) ∉ kCommandAt deniedParadigm := by
  constructor <;> decide
...
def corefPermittedAt_10a (t : Tree Unit String) (isPronA isPronB : Prop)
    [Decidable isPronA] [Decidable isPronB] (a b : TreePath) : Prop :=
  if isPronA ∧ ¬ isPronB then (a, b) ∉ kCommandAt t
  else if ¬ isPronA ∧ isPronB then (b, a) ∉ kCommandAt t
  else True
