...
so `few` comes out negative strong — B&C's Table II lists *few* as weak, a classification
...
* `exists_and_of_scopeUp_dual` — C10; `most_selfDual_of_odd_count`.
...
/-- A proper quantifier, or sieve, is one that some set passes and some set fails (§4.5). -/
def Sieve (Q : NP α) : Prop := (∃ X, Q X) ∧ ∃ X, ¬ Q X

/-- The principal filter generated by `B` consists of the sets containing `B` (§4.6). -/
def principalFilter (B : α → Prop) : NP α := λ X => ∀ x, B x → X x

variable {Q : NP α} {A B X : α → Prop}

/-- A quantifier living on `A` contains `A` iff it contains the domain (C1). -/
theorem livesOn_apply_iff (h : LivesOn Q A) : Q A ↔ Q (λ _ => True) := by
  rw [h (λ _ => True)]
  simp only [and_true]

/-- The principal filter generated by `B` lives on `A` iff `B ⊆ A` (C2). -/
...
/-- A mixed conjunction of an increasing and a decreasing quantifier, *John and no woman*, is
neither increasing nor decreasing (§4.10). -/
theorem not_monotone_mixed_conjunction :
    ¬ Monotone (individual true ⊓ restrict no (· = false)) ∧
    ¬ Antitone (individual true ⊓ restrict no (· = false)) := by
  constructor
  · intro h
...
/-- A determiner is strong when it is positive or negative strong (§4.6). -/
def Strong (D : GQ α) : Prop := PositiveStrong D ∨ NegativeStrong D

/-- A determiner is weak when it is neither positive nor negative strong (§4.6). -/
def Weak (D : GQ α) : Prop := ¬ Strong D

/-- A determiner is definite when every `D(A)` is the principal filter of a nonempty generator
(§4.6). -/
def Definite (D : GQ α) : Prop :=
  ∀ A, ∃ B : α → Prop, (∃ x, B x) ∧ ∀ X, D A X ↔ ∀ x, B x → X x

/-- Self-dual (§4.11). -/
def SelfDual (D : GQ α) : Prop := dual D = D

variable {D : GQ α}

/-- The *there*-sentence of a positive strong determiner is a tautology (§4.6), since *there are
...
/-- Definite determiners are positive strong (C3). -/
...
/-- Under the intersection condition, `B ∈ D(A)` iff the domain lies in `D(A ∩ B)`, the *there
is/are* paraphrase of weak determiners (C4). -/
...
/-- A strong determiner satisfying the intersection condition is trivial, since positive strong
makes every `D(A)` everything and negative strong makes it empty; so no proper strong
determiner is symmetric (C6). -/
...
/-- A persistent symmetric determiner is increasing (C7). -/
theorem scopeUp_of_restrictorUp_symmetric (hp : RestrictorMonotone D)
    (hs : Std.Symm D) : ScopeMonotone D :=
  fun R S S' hSS' hRS => hs.symm S' R (hp R hSS' (hs.symm R S hRS))

/-- A positive strong increasing determiner validates *D N that VP, VP*, since the scope lies
in `D` of the intersected restrictor (C8). -/
theorem apply_inter_of_positiveStrong_scopeUp (hps : PositiveStrong D)
    (hm : ScopeMonotone D) (A B : α → Prop) : D (λ x => A x ∧ B x) B :=
  hm _ (show (fun x => A x ∧ B x) ≤ B from fun _ h => h.2) (hps _)

/-- The dual of an increasing quantifier is increasing (C9 for the dual). -/
theorem scopeMonotone_dual (h : ScopeMonotone D) : ScopeMonotone (dual D) :=
  ScopeAntitone.compl _ (ScopeMonotone.innerNeg _ h)

/-- An increasing quantifier and its dual overlap, since `D(R)(A)` and `Ď(R)(B)` force a
common member of `A` and `B` (C10). -/
theorem exists_and_of_scopeUp_dual (hm : ScopeMonotone D) {R A B : α → Prop}
    (hA : D R A) (hB : dual D R B) : ∃ x, A x ∧ B x := by
  by_contra h
  push Not at h
  exact hB (hm R (show A ≤ (fun x => ¬ B x) from fun x hx hBx => h x hx hBx) hA)
...
open Classical in
theorem not_scopeUp_evenOutside : ¬ ScopeMonotone (evenOutside (α := Unit)) := by
  intro h
  have h0 : evenOutside (α := Unit) (λ _ => False) (λ _ => False) := by
    unfold evenOutside
    simp [count, countOn]
  have h1 := h (fun _ => False)
    (show ((fun _ : Unit => False) : Unit → Prop) ≤ fun _ => True from fun _ h => h.elim) h0
...
theorem restrictorUp_someNotAll : RestrictorMonotone (someNotAll (α := α)) :=
  λ _ _ _ hRR' ⟨⟨x, hx, hSx⟩, ⟨y, hy, hSy⟩⟩ =>
    ⟨⟨x, hRR' x hx, hSx⟩, ⟨y, hRR' y hy, hSy⟩⟩

theorem not_scopeUp_someNotAll : ¬ ScopeMonotone (someNotAll (α := Bool)) := by
  intro h
  have := h (fun _ => True)
    (show ((· = true) : Bool → Prop) ≤ fun _ => True from fun _ _ => trivial)
    ⟨⟨true, trivial, rfl⟩, ⟨false, trivial, by simp⟩⟩
  obtain ⟨-, y, -, hy⟩ := this
  exact hy trivial

theorem not_symm_someNotAll : ¬ Std.Symm (someNotAll (α := Bool)) := by
  intro h
  have := h.symm (fun _ => True) (· = true) ⟨⟨true, trivial, rfl⟩, ⟨false, trivial, by simp⟩⟩
  obtain ⟨-, y, -, hy⟩ := this
  exact hy trivial
...
theorem all_positiveStrong {d : GQ.Family} (h : d ∈ ⟦QuantityWord.all⟧) :
  ...
  exact positiveStrong_every

open Classical in
/-- *Most* is positive strong on nonempty restrictors (Table II; Appendix B's SP1), since *most
N are N* whenever there are `N`s. On the empty restrictor the proportional reading fails, as
in Barwise and Cooper's Table I column, so *most N* can denote the empty quantifier. -/
...
omit [Fintype α] in
/-- *Some* is weak (Table II), since *some N is a N* is contingent. -/
theorem some_weak [Nonempty α] : Weak (GQ.some : GQ α) := by
  rintro (h | h)
...
omit [Fintype α] in
/-- *No* is weak (Table II), since *no N is a N* is contingent. -/
theorem no_weak [Nonempty α] : Weak (no : GQ α) := by
  rintro (h | h)
  · exact h (λ _ => True) (Classical.arbitrary α) trivial trivial
  · exact h (λ _ => False) (λ x hx => hx.elim)

/-- *Exactly half*, the non-monotone determiner of §4.7, is weak, since *half the N are N* is
contingent, witnessed over `Fin 3`. -/
theorem half_weak : Weak (half : GQ (Fin 3)) := by
  rintro (h | h)
  · have key := h (fun x : Fin 3 => x = 0)
    simp only [half, count_eq_decidable (fun x : Fin 3 => x = 0 ∧ x = 0) _,
  ...
  · refine h (fun _ : Fin 3 => False) ?_
    simp only [half, count_eq_decidable (fun x : Fin 3 => False ∧ False) _,
...
open Classical in
/-- U6 for *exactly half*, the non-monotone example of §4.8, says that the non-monotone
determiner is the conjunction of an increasing and a decreasing one, as the monotonicity
constraint requires of simple NPs; `exactly n` decomposes likewise
(`Quantifier.GQ.exactly_eq_atLeast_inf_atMost`). -/
theorem half_eq_meet : (half : GQ α) = (atLeastHalf ⊓ atMostHalf) := by
  funext R S
  simp only [half, inf_apply, atLeastHalf, atMostHalf, eq_comm]
  exact propext le_antisymm_iff

open Classical in
theorem atLeastHalf_scopeUp : ScopeMonotone (atLeastHalf (α := α)) :=
  λ _ _ _ hSS' h => h.trans (Nat.mul_le_mul_left 2
    (count_le_of_imp (λ x hx => ⟨hx.1, hSS' x hx.2⟩)))

open Classical in
theorem atMostHalf_scopeDown : ScopeAntitone (atMostHalf (α := α)) :=
  λ _ _ _ hSS' h =>
    (Nat.mul_le_mul_left 2 (count_le_of_imp (λ x hx => ⟨hx.1, hSS' x hx.2⟩))).trans h

open Classical in
/-- On a restrictor of odd size, *more than half* is self-dual (§4.11). -/
theorem most_selfDual_of_odd_count {A : α → Prop} (h : Odd (count (λ x => A x)))
    (S : α → Prop) : dual (most : GQ α) A S ↔ most A S := by
  simp only [dual, innerNeg, compl_apply, most,
...
/-- C12 of [barwise-cooper-1981] (Appendix C p. 213) says that no first-order sentence over two
...
indices; atoms apply directly to variables, since the language has no function symbols. -/
...
/-- Barwise and Cooper's property (P) (p. 215) says that on models with `U ⊆ V` whose domain is
at least twice `|V|` plus the complexity of `ψ`, the `Q`-elimination `star` is correct, since
the domain "swamps out" `U` and `V`. -/
...
/-- C13 of [barwise-cooper-1981] (Appendix C pp. 214–215) says that even with the unrelativized
majority quantifier `Qx[·]`, "more than half of all things", no sentence of `L(Q)` is true in
exactly the finite models where more than half the `V`'s are `U`'s; the relativized quantifier
is not definable from the unrelativized one, so *most* must be treated as a determiner, not a
quantifier. Barwise and Cooper credit a related unpublished 1965 theorem to David Kaplan. -/
...
/-- The C12/C13 truth condition is `most` with `V` as restrictor — the
...
theorem mostUV_iff_most_sem {M : Type} [Fintype M] (U V : M → Prop) :
    MostUV U V ↔ Quantifier.GQ.most V U := by
  classical
  unfold MostUV Quantifier.GQ.most
