...
private def markOf (c s : RoseTree SOLabel) : Option Marked :=
...
def stages : List Stage :=
  [⟨⟨lexLeaf tokN, []⟩, leafP tokN, [], 0⟩].flatMap (step tokA) |>.flatMap (step tokNum)
    |>.flatMap (step tokDem)
