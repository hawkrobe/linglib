...
def entityIntro (n : ℕ) (body : Update (HAssign World Atom)) : Update (HAssign World Atom) :=
  {(g, h) | ∃ x : Individual Atom, Function.update g n (.entity x) ~[body] h}
...
theorem entity_trapped_by_test {n : ℕ} {C : Condition (HAssign World Atom)}
    {g h : HAssign World Atom} (hNovel : g n = .undef) (hTest : g ~[test C] h) : h n = .undef :=
...
(5c). -/
theorem them_after_doesntOwnADog {h : HAssign Unit Ent} (hNeg : g₀ ~[doesntOwnADog] h) :
