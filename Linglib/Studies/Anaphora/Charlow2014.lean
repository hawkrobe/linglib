import Linglib.Studies.Anaphora.KampReyle1993
...
namespace Studies.Anaphora.Charlow2014
...
def cdrt : AnaphoraFramework farmer donkey owns beats where
  ...
  donkeyHolds := ∀ i : Register E,
    DProp.true_at (Studies.Anaphora.Cooper2023.cdrt_full_donkey
  ...
  donkey_iff_classical := by
    ...
    · intro h x y hf hd ho
      have := h (fun _ => x)
      rw [Studies.Anaphora.Cooper2023.full_donkey_equiv] at this
      exact ((this.some) x hf y hd ho).down
    · intro h i
      rw [Studies.Anaphora.Cooper2023.full_donkey_equiv]
...
def ttr : AnaphoraFramework farmer donkey owns beats where
  ...
  donkeyHolds := Nonempty (Studies.Anaphora.Cooper2023.ttr_full_donkey
                            farmer donkey owns beats)
  donkey_iff_classical := by
    unfold classicalDonkey Studies.Anaphora.Cooper2023.ttr_full_donkey
...
end Studies.Anaphora.Charlow2014
