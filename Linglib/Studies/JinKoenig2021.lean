...
-- ════════════════════════════════════════════════════
-- § 1.6. Table 3: EN Languages and Trigger Concepts
-- ════════════════════════════════════════════════════
...
def enLanguages : List ENLanguageEntry :=
  ...
  , ⟨"Albanian",        "sqi", "Albanian",   .eurasia, ["FEAR"]⟩
  , ⟨"Latvian",         "lvs", "Baltic",     .eurasia, ["FEAR"]⟩
  , ⟨"Lithuanian",      "lit", "Baltic",     .eurasia, ["FEAR", "UNTIL"]⟩
  , ⟨"Basque",          "eus", "Basque",     .eurasia, ["FEAR", "FORBID"]⟩
  , ⟨"Tshangla",        "tsj", "Bodic",      .eurasia, ["UNTIL"]⟩
  , ⟨"Rabha",           "rah", "Bodo-Garo",  .eurasia, ["UNTIL"]⟩
  , ⟨"Burushaski",      "bsk", "Burushaski", .eurasia, ["UNTIL"]⟩
  ...
  , ⟨"Komi-Zyrian",     "kpf", "Finnic",     .eurasia, ["FEAR"]⟩
  , ⟨"Livonian",        "liv", "Livonian",   .eurasia, ["THAN"]⟩
  , ⟨"Afrikaans",       "afr", "Germanic",   .eurasia, ["BEFORE", "UNLESS"]⟩
  ...
  , ⟨"Greek",           "ell", "Greek",      .eurasia,
    ["BEWARE", "FEAR", "WORRY"]⟩
  , ⟨"Bengali",         "ben", "Indic",      .eurasia, ["UNTIL"]⟩
  , ⟨"Domari",          "rmt", "Indic",      .eurasia, ["PREVENT"]⟩
  , ⟨"Hindi",           "hin", "Indic",      .eurasia, ["FEAR", "UNTIL"]⟩
  , ⟨"Kashmiri",        "kas", "Indic",      .eurasia, ["UNTIL"]⟩
  , ⟨"Punjabi",         "pan", "Indic",      .eurasia, ["UNTIL"]⟩
  , ⟨"Persian",         "fas", "Iranian",    .eurasia, ["UNTIL/UNLESS"]⟩
  ...
  , ⟨"Japanese",        "jpn", "Japanese",   .eurasia, ["BEFORE"]⟩
  , ⟨"Kadu",            "zkd", "Jingpho-Luish", .eurasia, ["BEFORE"]⟩
  , ⟨"Lao",             "lao", "Kam-Tai",    .eurasia, ["FORBID"]⟩
  , ⟨"Georgian",        "kat", "Kartvelian",  .eurasia,
    ["FEAR", "UNTIL"]⟩
  , ⟨"Korean",          "kor", "Korean",     .eurasia,
    ["BEFORE", "IT'S BEEN A LONG TIME SINCE"]⟩
  , ⟨"Karbi",           "mjw", "Kuki-Chin",  .eurasia, ["BEFORE"]⟩
  , ⟨"Ingush",          "inh", "Nakh",       .eurasia, ["ONLY"]⟩
  , ⟨"Kurux",           "kxl", "Northern Dravidian", .eurasia,
    ["NOT WITHOUT", "UNTIL"]⟩
  , ⟨"Abkhaz",          "abk", "Northwest Caucasian", .eurasia, ["FEAR"]⟩
  , ⟨"Daakaka",         "bpa", "Oceanic",    .eurasia, ["FEAR", "LACK"]⟩
  , ⟨"Samoan",          "smo", "Oceanic",    .eurasia,
    ["AFRAID", "DISLIKE"]⟩
  , ⟨"Tuvaluan",        "tvl", "Oceanic",    .eurasia, ["BEFORE"]⟩
  , ⟨"Vaeakau-Taumako", "piv", "Oceanic",    .eurasia, ["CRITICIZE"]⟩
  , ⟨"Bulgarian",       "bul", "Slavic",     .eurasia, ["UNTIL"]⟩
  , ⟨"Czech",           "cse", "Slavic",     .eurasia, ["FEAR"]⟩
  ...
  , ⟨"Turkish",         "tur", "Turkic",     .eurasia,
    ["BEFORE", "WITHOUT/FROM"]⟩
  , ⟨"Kumyk",           "kum", "Turkic",     .eurasia, ["FEAR"]⟩
  , ⟨"Hungarian",       "hun", "Ugric",      .eurasia, ["UNTIL"]⟩
  , ⟨"Vietnamese",      "vie", "Viet-Muong", .eurasia,
    ["AVOID", "FORBID", "FORGET", "REFUSE", "STOP"]⟩
  -- ── North America (1 language) ─────────────────────────────────────
  , ⟨"Lakhota",         "lkt", "Core Siouan", .northAmerica, ["BEFORE"]⟩
  -- ── Pidgins & Creoles (4 languages) ────────────────────────────────
  , ⟨"Haitian creole",  "hat", "Pidgin/Creole", .pidginsCreoles, ["WITHOUT"]⟩
  , ⟨"Nigerian pidgin", "pcm", "Pidgin/Creole", .pidginsCreoles, ["UNTIL"]⟩
  , ⟨"Pichinglis",      "fpe", "Pidgin/Creole", .pidginsCreoles, ["FEAR"]⟩
...
private def hasBeforeOrUntil (triggers : List String) : Bool :=
  triggers.any (fun t =>
    containsSubstr' t "BEFORE" || containsSubstr' t "UNTIL")

private def hasFearOrAfraid (triggers : List String) : Bool :=
  triggers.any (fun t =>
    containsSubstr' t "FEAR" || containsSubstr' t "AFRAID" ||
