...
`r` and every distractor failing some pair ((3), conditions C1 and C2): the first submaxim
of Quantity over `extension`. -/
abbrev Distinguishing (kb : KB E A V) (r : E) (C : Finset E) (L : Description A V) : Prop :=
  Pragmatics.GriceanMaxims.Distinguishes (extension kb) C r L
...
theorem distinguishing_iff {kb : KB E A V} {r : E} {C : Finset E} {L : Description A V} :
  ...
  simp [Pragmatics.GriceanMaxims.Distinguishes, extension]
...
/-- Reiter's first rule is subset-minimality among distinguishing descriptions, the second
