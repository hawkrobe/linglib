...
@cite{faust-lampitelli-2026}'s headline claim: in the Ethiosemitic
...
(@cite{kaye-lowenstamm-vergnaud-1985}, @cite{backley-2011}) of
...
* Templatic morphology beyond bare CV patterns (see
  @cite{faust-2014}, @cite{faust-2017b}, @cite{faust-lampitelli-2023}).
...
* The OCP merger operation (`Phonology.Tier.ocpCollapse`) unifies
  this paper's |A|+|A| fusion with @cite{lionnet-2022}'s TRN merger
...
/-- A V-slot is *synersis-licensed silent* iff (a) it is empty
    ...
    @cite{faust-lampitelli-2026} eq. (23)-(28). -/
...
/-- **Self-licensing blocks dissociation**
    @cite{faust-lampitelli-2026} eq. (28a-c). When the fused |A| at
...
/-- **Economy drives synersis** @cite{faust-lampitelli-2026}
...
/-- **T4 — Walker & Rose 2015 overshoots**
    @cite{faust-lampitelli-2026} §3.3.2 critique of
    @cite{walker-rose-2015-amp}. Three refutation legs (the third
    ...
    not a published paper @cite{walker-rose-2015-amp} note 22). -/
...
* **Ejectives also contain |A| but never undergo syneresis** (paper
  n. 8, page 9). Per @cite{lowenstamm-prunet-1988} and
  @cite{faust-2017b}, ejectives also lower /ʌ/ to [a], suggesting
  ejectives also bear the |A| element; per @cite{bellem-2007}:102-153
...
* **The Tigre [samʕa] underlying form** (paper n. 11, page 11). The
  ...
  [samʕa] 'he heard' and [fagra] 'he left' per @cite{raz-1980}). The
...
* **The supposed weakness of gutturals lacks an external definition**
  ...
  satisfactorily, citing only sketches in @cite{angoujard-1995} and
  @cite{walker-rose-2015-amp}.
...
* **The realised vowel position in /tisʌħib/ → [tisɨħib] and
  ...
  position", tentatively invoking @cite{buckley-2000}'s
