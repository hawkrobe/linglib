...
* Identification is `Reference.Distinguishes` over the extension of a description in a
  display, against every other object. Over-description is measured against the bare noun, the
  paper's one contrast of modified with bare target. Production rates, ratings and fixation
  proportions stay in the prose above.
...
/-- An over-description of `r`: the description identifies it with a modifier its bare noun
does not need. -/
...
apple already rests on a towel, *the other towel* identifies it without over-describing, and
*the box* is identified bare. -/
...
/-- Over the one-referent display, every target is identified and the modified target of (5)
and (6) is an over-description. -/
