...
the qualitative claim (identity is dispreferred).

TODO: offer a categorical `¬IDENT` variant — the indicator that `identViol = 0`
(1 iff fully identical), or its per-feature analogue — matching [alderete-1999]'s
"negation of F" definition, and use it where the minimal-differentiation
prediction (change exactly one feature) is load-bearing.
-/
