...
* The Best Landmark Principle of Hudson's later theory and the paper's objection to it from
  extraction, (11) to (14), are not formalized.
...
verb labelled xc in (9), and extractee. -/
inductive Rel where
  | subj
  | obj
  | comp
...
/-- The landmark graph: an arc from each word's landmark to the word, labelled with UD's
unspecified relation. -/
def landmarkGraph : Graph n where
  words := A.words
  label p w := if A.IsLandmark w p then some .dep else none
  root := A.root

/-- The graph in which every parent is a landmark, as in the diagrams (52) to (55). -/
def parentGraph : Graph n where
  words := A.words
...
landmark. -/
def ExtractionByLandmark : Prop := ∀ h w, A.Dep h w .extractee → A.IsLandmark w h
...
/-- Word order by the 2007 theory: the landmarks form a projective tree, every word stands on
its side of its landmark, and an extracted word's landmark is the head it is extracted from. -/
def OrderedByLandmarks : Prop :=
  A.landmarkGraph.IsTree ∧ A.landmarkGraph.IsProjective ∧ A.RespectsLandmarks ∧
    A.ExtractionByLandmark

/-- Word order by the revised theory: the landmarks form a projective tree, every word stands on
its side of its landmark, and every extracted word precedes its positional head. -/
def OrderedByPositionalHeads : Prop :=
  A.landmarkGraph.IsTree ∧ A.landmarkGraph.IsProjective ∧ A.RespectsLandmarks ∧
    A.RespectsPositionalHeads

instance : Decidable A.RespectsLandmarks := inferInstanceAs (Decidable (∀ _ _, _))
instance : Decidable A.ExtractionByLandmark := inferInstanceAs (Decidable (∀ _ _, _))
...
@[simp] theorem landmarkGraph_adj : A.landmarkGraph.Adj p w ↔ A.IsLandmark w p := by
  simp only [Graph.Adj, landmarkGraph]
  split_ifs <;> simp [*]

@[simp] theorem parentGraph_adj : A.parentGraph.Adj p w ↔ A.Parent p w := by
  simp only [Graph.Adj, parentGraph]
  split_ifs <;> simp [*]

/-- Two words that depend on each other cannot each be the other's landmark: by transitivity
each would be its own. -/
theorem not_isTree_parentGraph (h₁ : A.Parent p w) (h₂ : A.Parent w p) :
    ¬ A.parentGraph.IsTree := fun hT ↦
  not_adj_dominates hT.acyclic (parentGraph_adj.2 h₁) (.single (parentGraph_adj.2 h₂))
...
theorem not_extractionByLandmark (hex : A.Dep h w .extractee)
    (hs : A.SubordinateParent w h) : ¬ A.ExtractionByLandmark := fun hE ↦
...
theorem not_orderedByLandmarks_of_extraction (hex : A.Dep h w .extractee) (hq : A.Parent q w)
  ...
  not_extractionByLandmark hex ⟨⟨_, hex⟩, q, hqh, hq, .head hq (.single hwh)⟩ hO.2.2.2
