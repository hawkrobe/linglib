...
operation with no target does not converge (37)–(40). `amalgamateSpine` runs this
...
settings of the `[m]` features in (41) put one and the same verbal complex at T (French),
...
the moved Neg in Russian polarity-focus VSO, and, in (68), a complex head that skips two
...
Bulgarian long head movement of (29)–(33), which crosses auxiliaries that remain separate
...
the paper's argument. The English complex of (41) is built without a Neg head, as the
parenthesis there indicates.

## TODO

* Chain Reduction, and the Hebrew bare-infinitive fronting of (67), where the moved V
...
/-- (43): the Russian spine, V, v and Asp Raising and T Lowering. -/
...
/-! ### Verb second and polarity focus (§4.1) -/

/-- (59a): the German embedded clause, V Raising into T. -/
def germanEmbedded : List (Amalgam ClausalHead) := [single V (some .raising), single T none]

/-- (61a): the Danish embedded clause, T Lowering into v and v into V. -/
def danishEmbedded : List (Amalgam ClausalHead) :=
  [single V none, single v (some .lowering), single T (some .lowering)]

/-- (63): Russian SVO, the verbal complex amalgamated at Neg below the subject in Spec,TP. -/
...
/-- (59b–c): in the German root clause T moves to the C domain and V Raises into its low
...
/-- (62): in the Danish root clause V moves to the C domain and T and v Lower into its low
...
/-- (65): Russian VSO under polarity focus, Neg moving to Pol and the verbal complex
...
/-- (68), with H1 to H6 as `0` to `5`: the moved H1 occupies the bottom and the top of the
