...
theorem κz_softmax_concentrates :
    ∀ w, Filter.Tendsto (fun (α : ℝ) => Core.softmax (α • rankToScore κ_z) w)
...
theorem condProb_birds_fly :
    Filter.Tendsto
      (fun α => RSA.RankingBridge.condProb
        (rankToScore κ_z) α isBird flies)
      Filter.atTop (nhds 1) :=
  RSA.RankingBridge.condProb_tendsto_one
    κ_z isBird flies birds_fly ⟨.birdFlies, trivial⟩
...
theorem condProb_penguins_dont_fly :
    Filter.Tendsto
      ...
      Filter.atTop (nhds 1) :=
  RSA.RankingBridge.condProb_tendsto_one κ_z
    (fun w => isPenguin w ∧ isBird w) (fun w => ¬ flies w)
