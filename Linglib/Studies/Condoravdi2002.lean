...
def atEvent (P : EventPred W Time) : IntervalPred W Time := PRFV P
...
def atState (P : EventPred W Time) : IntervalPred W Time :=
  fun w t => ∃ e : Event Time, e.τ.overlaps t ∧ P w e

/-- The AT relation, dispatching on eventuality sort. The paper's third case
    (properties of times) is vacuous here: `EventPred` is eventuality-valued. -/
def at' (sort : Dynamicity) (P : EventPred W Time) (w : W) (t : Interval Time) :
    Prop :=
  match sort with
...
theorem atState_of_atEvent (P : EventPred W Time) (w : W) (t : Interval Time)
    (h : atEvent P w t) : atState P w t :=
  let ⟨e, hSub, hP⟩ := h
  ⟨e, Interval.subinterval_overlaps hSub, hP⟩

/-- `atEvent` is monotone in the reference interval. -/
theorem atEvent_mono {t₁ t₂ : Interval Time} (P : EventPred W Time) (w : W)
...
theorem atState_mono {t₁ t₂ : Interval Time} (P : EventPred W Time) (w : W)
...
def atForward (sort : Dynamicity) (P : EventPred W Time) (w : W) (t : Time) :
    Prop :=
  match sort with
...
theorem atEventForward_of_atEvent_point (P : EventPred W Time) (w : W) (t : Time)
    (h : atEvent P w (Interval.point t)) : atEventForward P w t :=
  let ⟨e, hSub, hP⟩ := h
  ⟨e, hSub.1, hP⟩

/-- `atState` at a point `[t, t]` implies `atStateForward` at `t`. -/
theorem atStateForward_of_atState_point (P : EventPred W Time) (w : W) (t : Time)
...
theorem atState_of_impf (P : EventPred W Time) (w : W) (t : Interval Time)
