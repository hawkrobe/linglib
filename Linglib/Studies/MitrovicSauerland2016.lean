...
@cite{mitrovic-sauerland-2016} @cite{mitrovic-sauerland-2014} @cite{mitrovic-2021}
...
- Some languages overtly realise both heads, giving triadic exponency
  (J + two μ): @cite{mitrovic-sauerland-2016} §3.1 lists SE Macedonian,
...
* `mu_boundness_asymmetry` — Georgian μ is bound, Hungarian μ is free; @cite{bill-etal-2025}
...
* Language records (`english`, `japanese`, `hungarian`, `georgian`, `latin`,
  ...
  follows @cite{mitrovic-2021}; @cite{mitrovic-sauerland-2016} itself
...
/-- **MU is additive.** Every language with a MU conjunction particle uses
    the same morpheme as its additive ("also/too") particle (@cite{mitrovic-sauerland-2016}
...
/-- **J is universal in the M&S focus sample.** Every M&S-classified language
    has at least the J-only strategy (@cite{mitrovic-sauerland-2016} §3). -/
...
/-- **MU boundness asymmetry.** Georgian MU (*-c*) is bound; Hungarian MU
    (*is*) is free. @cite{bill-etal-2025} (with @cite{mitrovic-2021})
