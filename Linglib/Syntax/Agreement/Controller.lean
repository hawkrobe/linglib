...
While `Syntax/Agreement/Hierarchy.lean` enumerates WHERE agreement morphology
...
(`Controller.subj` × `Position.predicate`) and possessor-controlled
attributive agreement (`Controller.poss` × `Position.attributive`).
...
- **Case** — case-marking (nom/erg/abs/dat/...) is orthogonal to
  ...
  phenomena. Case lives in `Syntax/Case/`.
