...
theorem Cut.extractionCost_add_quotientCost (p : Forest (UnorderedTree (α ⊕ β)) × UnorderedTree
    (α ⊕ β)) :
...
@[simp] theorem imNetCost_eq_zero (p : Forest (UnorderedTree (α ⊕ β)) × UnorderedTree (α ⊕ β)) :
    imNetCost p = 0 := by
  rw [imNetCost, Cut.extractionCost_add_quotientCost]; rfl

@[simp] theorem swNetCost_eq_depthC (p : Forest (UnorderedTree (α ⊕ β)) × UnorderedTree (α ⊕ β)) :
...
variable [DecidableEq (UnorderedTree (α ⊕ β))]

/-- The **ε-weighted Δ^c Merge operator** (MCB §1.5, eq 1.5.2): the Δ^c merge
    scaled by the Minimal-Search weight `ε^c`. -/
noncomputable def mergeOpCEps (τ : UnorderedTree (α ⊕ β) → β) (ε : R) (c : ℕ)
    (lbl : α ⊕ β) (S S' : UnorderedTree (α ⊕ β)) :
...
/-- At ε = 1 the weight is trivial and `mergeOpCEps` recovers the unweighted Δ^c
    Merge. -/
@[simp] theorem mergeOpCEps_one (τ : UnorderedTree (α ⊕ β) → β) (c : ℕ)
    (lbl : α ⊕ β) (S S' : UnorderedTree (α ⊕ β)) :
...
@[simp] theorem mergeOpCEps_zero_em (τ : UnorderedTree (α ⊕ β) → β)
    (lbl : α ⊕ β) (S S' : UnorderedTree (α ⊕ β)) :
...
@[simp] theorem mergeOpCEps_zero_im (τ : UnorderedTree (α ⊕ β) → β)
...
theorem mergeOpCEps_zero_sideward (τ : UnorderedTree (α ⊕ β) → β) {c : ℕ} (hc : 0 < c)
    (lbl : α ⊕ β) (S S' : UnorderedTree (α ⊕ β)) :
