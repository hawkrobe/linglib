...
-- ════════════════════════════════════════════════════
-- § 5. Homogeneity and subinterval properties
-- ════════════════════════════════════════════════════
...
/-- Whether a predicate has the subinterval property (qualified).
    ...
    - **States** have the *full* SIP: every subinterval of a knowing event
      is a knowing event (@cite{smith-1997}).
    - **Activities** have a *qualified* SIP: subintervals down to a
      ...
      single stride). See `VendlerClass.hasFullSubintervalProp` for
      the distinction.

    UNVERIFIED: Smith 1997 page reference for full-SIP claim cited from memory. -/
def isHomogeneous (p : AspectualProfile) : Bool :=
...
-- ════════════════════════════════════════════════════
-- § 6. Atomic distributivity
-- ════════════════════════════════════════════════════

namespace VendlerClass

/-- Whether a VendlerClass predicts ATOM-DIST_t (@cite{zhao-2025}).
    ...
    UNVERIFIED: Zhao 2025 Def 5.3 reference cited from memory. -/
def predictsAtomDist : VendlerClass → Bool
  | .state => true
  | .activity | .achievement | .accomplishment | .semelfactive => false
