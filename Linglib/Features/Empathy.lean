...
Empathy in the sense of [kuno-1987]: the speaker's identification with
...
([charnavel-mateu-2015]); in Japanese, transferring verbs *yaru/kureru*
lexically alternate by speaker-empathy ([kuno-1987] p. 246).

This file provides the minimal primitive. Frameworks that *use* empathy
(e.g., [charnavel-mateu-2015]'s antilogophoricity hierarchy) live
...
/-- A binary empathy property. The speaker either identifies with the
    ...
    graded variants (the Empathy Hierarchy of [kuno-1987] ch. 5) can
    be added when downstream consumers need them. -/
def IsEmpathyLocus (p : Prop) : Prop := p

/-- Kuno's *Speech-Act Empathy Hierarchy* ([kuno-1987] p. 207):
    ...
    empathy locus, per [charnavel-mateu-2015]) is downstream. -/
