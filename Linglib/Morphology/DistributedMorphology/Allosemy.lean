...
inventory is the head's contentful allosemes with a zero adjoined: `Option`, in the `WithZero` pattern, with `none`
...
/-- The Voice allosemes as a competing exponence vocabulary: engineer for
    a saturated eventive VoiceP complement (most specified), holder for a stative one, expletive elsewhere (the
