...
structure RSAConfigData (U W : Type*) [Fintype U] [Fintype W]
  ...
  /-- Optional QUD projection: sums L0 over equivalence classes before S1
      scoring. When `some project`, L0(w|u) becomes Σ_{w'∼w} L0(w'|u). -/
  qudProject : Option (W → Latent → ℕ) := none
...
noncomputable def effectiveL0 [DecidableEq L]
    (qudProj : Option (W → L → ℕ)) (l : L) (l0 : U → W → ℝ) : U → W → ℝ :=
  match qudProj with
  | none => l0
  | some project => fun u w => qudProjectR project l l0 u w

set_option linter.unusedSectionVars false in
theorem effectiveL0_nonneg [DecidableEq L]
    (qudProj : Option (W → L → ℕ)) (l : L)
    (l0 : U → W → ℝ) (hl0 : ∀ u w, 0 ≤ l0 u w) (u : U) (w : W) :
    0 ≤ effectiveL0 qudProj l l0 u w := by
  match qudProj with
  | none => exact hl0 u w
  | some project =>
    simp only [effectiveL0, qudProjectR]
    exact Finset.sum_nonneg fun w' _ => hl0 u w'
...
noncomputable def RSAConfigData.toRSAConfig [DecidableEq U] [DecidableEq W]
  ...
  Latent := d.Latent
  meaning _ l u w := ↑(d.meaning l u w)
  meaning_nonneg _ l u w := by exact_mod_cast d.meaning_nonneg l u w
  s1Score l0 α l w u := d.s1Spec.toS1Score (effectiveL0 d.qudProject l l0) α l w u
  s1Score_nonneg l0 α l w u hl0 hα :=
    d.s1Spec.toS1Score_nonneg _ α l w u
      (fun u' w' => effectiveL0_nonneg d.qudProject l l0 hl0 u' w') hα
  α := ↑d.α
...
theorem RSAConfigData.toRSAConfig_eq [DecidableEq U] [DecidableEq W]
    ...
    (h_meaning : @HEq (Unit → d.Latent → U → W → ℝ) d.toRSAConfig.meaning
                       (cfg.Ctx → cfg.Latent → U → W → ℝ) cfg.meaning)
  ...
  cases cfg with
  | mk C L m mnn s snn a ap lp lpnn wp wpnn tr ini =>
    simp only at h_ctx
    subst h_ctx h_lat
