...
def fallingDeclarative (s : GunlogsonState W) (p : Set W) : GunlogsonState W :=
...
def risingDeclarative (s : GunlogsonState W) (p : Set W) : GunlogsonState W :=
  { s with addresseeSlate := s.addresseeSlate.add p .otherGenerated }

/-- Assert: defaults to falling declarative (standard assertion). -/
def assert (s : GunlogsonState W) (p : Set W) : GunlogsonState W :=
...
def canChallenge (src : CommitmentSource) (role : DiscourseRole) : Prop :=
  match src, role with
...
instance (src : CommitmentSource) (role : DiscourseRole) :
    Decidable (canChallenge src role) := by
  unfold canChallenge
  cases src <;> cases role <;> exact inferInstance
...
def cbcMet (s : GunlogsonState W) (p : Set W) : Prop :=
...
def risingDeclarativeFelicitous (s : GunlogsonState W) (p : Set W)
...
def uninformativeForAddressee (s : GunlogsonState W) (p : Set W) : Prop :=
...
theorem rising_uninformative_for_speaker (s : GunlogsonState W) (p : Set W) :
...
theorem cbc_from_uninformativeness (s : GunlogsonState W) (p : Set W) :
...
def confirm (s : GunlogsonState W) (p : Set W) : GunlogsonState W :=
...
def strongDeny (s : GunlogsonState W) (p : Set W) : GunlogsonState W :=
...
theorem rising_confirm_has_both {W : Type*} (p : Set W) :
