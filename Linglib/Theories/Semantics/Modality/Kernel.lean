...
private theorem propIntersection_cons (φ : (World → Prop)) (A : List ((World → Prop))) :
...
structure Kernel where
  props : List ((World → Prop))

namespace Kernel

/-- B_K = ⋂K. -/
def base (k : Kernel) : Set World :=
...
def followsFrom (k : Kernel) (φ : (World → Prop)) : Prop :=
  Kratzer.followsFrom φ k.props

/-- φ is compatible with K iff B_K ∩ ⟦φ⟧ ≠ ∅. -/
def compatibleWith (k : Kernel) (φ : (World → Prop)) : Prop :=
...
(Implementation 1). -/
def directlySettlesExplicit (k : Kernel) (φ : (World → Prop)) : Prop :=
  ∃ x ∈ k.props,
...
/-- Refine a partition along φ-boundaries (@cite{von-fintel-gillies-2010} subject matter).
    ...
    `World → Prop` without a `[DecidablePred]` constraint that would
    interfere with `List.foldl`. -/
noncomputable def refine (partition : List (List World)) (φ : (World → Prop)) :
    List (List World) :=
...
noncomputable def kernelPartition (k : Kernel) : List (List World) :=
  k.props.foldl refine [allWorlds]

/-- K settles P via partition iff P is a union of cells in S_K
(Implementation 2). -/
def settlesByPartition (k : Kernel) (φ : (World → Prop)) : Prop :=
...
private theorem mem_propExtension_of_propIntersection
    {x : (World → Prop)} {props : List ((World → Prop))}
...
/-- What both implementations share: settling implies entailment.
    ...
    B_K ⊆ X ⊆ ⟦¬φ⟧. -/
theorem explicit_implies_entailment (k : Kernel) (φ : (World → Prop))
...
private theorem refine_preserves_colocated
    (partition : List (List World)) (p : (World → Prop))
...
private theorem foldl_refine_preserves_colocated
    (props : List ((World → Prop))) (partition : List (List World))
...
private theorem colocated_same_phi
    (partition : List (List World)) (φ : (World → Prop))
...
private theorem refine_cell_uniform_p
    (partition : List (List World)) (p : (World → Prop))
    {cell : List World} (h_cell : cell ∈ refine partition p)
    {w v : World} (hw : w ∈ cell) (hv : v ∈ cell) :
    (p w ↔ p v) := by
  classical
...
private theorem refine_preserves_uniformity
    (partition : List (List World)) (p q : (World → Prop))
...
private theorem foldl_refine_cell_uniform
    (props : List ((World → Prop)))
    {cell : List World} (h_cell : cell ∈ props.foldl refine [allWorlds])
    {w v : World} (hw : w ∈ cell) (hv : v ∈ cell)
    {q : (World → Prop)} (hq : q ∈ props) :
    (q w ↔ q v) := by
  have := foldl_refine_uniform_aux props [allWorlds] []
...
private theorem kernelPartition_cells_uniform (k : Kernel)
    {cell : List World} (h_cell : cell ∈ kernelPartition k)
    {w v : World} (hw : w ∈ cell) (hv : v ∈ cell)
    {q : (World → Prop)} (hq : q ∈ k.props) :
    (q w ↔ q v) :=
  foldl_refine_cell_uniform k.props h_cell hw hv hq

/-- compatible iff B_K ∩ ⟦φ⟧ ≠ ∅. -/
private theorem compatibleWith_iff_exists (k : Kernel) (φ : (World → Prop)) :
...
private theorem followsFrom_iff_forall (k : Kernel) (φ : (World → Prop)) :
...
theorem partition_implies_entailment (k : Kernel) (φ : (World → Prop))
  ...
  have hColoc : ∀ v ∈ k.base, CoLocated (kernelPartition k) w₀ v := by
    ...
    · -- Initially co-located in [allWorlds]
      exact ⟨allWorlds, List.mem_singleton.mpr rfl,
        Semantics.Attitudes.Intensional.mem_allWorlds w₀, Semantics.Attitudes.Intensional.mem_allWorlds v⟩
    · exact hAgree v hv
...
def kernelMust (k : Kernel) (φ : (World → Prop)) : PrProp World where
  presup := λ _ => ¬ directlySettlesExplicit k φ
  assertion := λ _ => k.followsFrom φ

/-- ⟦might φ⟧: presupposes K doesn't settle φ; asserts B_K ∩ ⟦φ⟧ ≠ ∅. -/
def kernelMight (k : Kernel) (φ : (World → Prop)) : PrProp World where
  presup := λ _ => ¬ directlySettlesExplicit k φ
  assertion := λ _ => k.compatibleWith φ

/-- ⟦can't φ⟧ = must(¬φ). -/
def kernelCant (k : Kernel) (φ : (World → Prop)) : PrProp World :=
  kernelMust k (λ w => ¬ φ w)

/-- World-dependent ⟦must φ⟧. -/
def kernelMustW (kb : KernelBackground) (φ : (World → Prop)) : PrProp World where
  presup := λ w => ¬ directlySettlesExplicit (kb w) φ
  assertion := λ w => (kb w).followsFrom φ

/-- World-dependent ⟦might φ⟧. -/
def kernelMightW (kb : KernelBackground) (φ : (World → Prop)) : PrProp World where
  presup := λ w => ¬ directlySettlesExplicit (kb w) φ
...
theorem must_is_strong (k : Kernel) (φ : (World → Prop)) (w : World)
...
theorem must_entails_prejacent (k : Kernel) (φ : (World → Prop)) (w : World)
    (hReal : w ∈ k.base)
    (_hDef : (kernelMust k φ).presup w)
...
theorem kernel_duality (k : Kernel) (φ : (World → Prop)) (w : World) :
...
theorem empty_kernel_always_defined (φ : (World → Prop)) (w : World) :
...
theorem kernelMust_eq_simpleNecessity (k : Kernel) (φ : (World → Prop)) (w : World) :
    (kernelMust k φ).assertion w ↔
      simpleNecessity k.toModalBase φ w := by
  show k.followsFrom φ ↔ _
  simp only [Kernel.followsFrom, Kratzer.followsFrom,
...
theorem kernelMust_eq_necessity (k : Kernel) (φ : (World → Prop)) (w : World) :
    (kernelMust k φ).assertion w ↔
...
theorem kernelMust_eq_epistemicNecessity (k : Kernel) (φ : (World → Prop)) (w : World) :
    (kernelMust k φ).assertion w ↔
...
theorem kernelMustW_eq_necessity (kb : KernelBackground) (φ : (World → Prop)) (w : World) :
...
theorem explicit_not_implies_partition :
  ...
  · -- ¬ settlesByPartition: .w1 and .w2 are co-located but disagree on redOrBlue.
    ...
    have hColoc : CoLocated (kernelPartition ⟨[red]⟩) .w1 .w2 := by
      apply foldl_refine_preserves_colocated
      · exact ⟨allWorlds, List.mem_singleton.mpr rfl,
          Semantics.Attitudes.Intensional.mem_allWorlds (.w1 : World), Semantics.Attitudes.Intensional.mem_allWorlds (.w2 : World)⟩
      · intro p hp
...
theorem modus_ponens_with_must (k : Kernel) (φ ψ : (World → Prop)) (w : World)
    (hReal : w ∈ k.base)
...
theorem must_perhaps_contradiction (k : Kernel) (φ : (World → Prop)) (w : World)
    (_hDef : (kernelMust k φ).presup w)
...
theorem direct_evidence_infelicity (k : Kernel) (φ : (World → Prop)) (w : World)
...
theorem settling_monotone (k : Kernel) (p : (World → Prop)) (φ : (World → Prop))
    (hSettled : directlySettlesExplicit k φ) :
...
theorem cant_might_exclusion (k : Kernel) (φ : (World → Prop)) (w : World)
...
theorem cant_entails_negation (k : Kernel) (φ : (World → Prop)) (w : World)
    (hReal : w ∈ k.base)
...
theorem cant_evidential_parallel (k : Kernel) (φ : (World → Prop)) (w : World) :
...
theorem cant_eq_must_neg (k : Kernel) (φ : (World → Prop)) (w : World) :
...
abbrev Background := List ((World → Prop))

/-- B_U = ⋂U: worlds compatible with the prior information state. -/
def backgroundBase (u : Background) : Set World :=
  propIntersection u
...
`[DecidablePred φ]`. -/
def evidenceRaises (p φ : (World → Prop))
    [DecidablePred p] [DecidablePred φ] : Prop :=
  let pCount := ((Finset.univ : Finset World).filter p).card
  let phiInP := ((Finset.univ : Finset World).filter (fun w => p w ∧ φ w)).card
  let phiInAll := ((Finset.univ : Finset World).filter φ).card
  phiInP * (Finset.univ : Finset World).card > phiInAll * pCount
...
/-- Some proposition in K raises the probability of φ (condition i).
    ...
    `World → Prop`. -/
def Kernel.evidenceSupports (k : Kernel) (φ : (World → Prop))
    [DecidablePred φ] : Prop :=
  haveI : ∀ p : World → Prop, DecidablePred p :=
...
def nandaoFelicitous (k : Kernel) (u : Background) (φ : (World → Prop))
    [DecidablePred φ] : Prop :=
  k.evidenceSupports φ ∧ k.incompatibleWith u ∧ ¬ directlySettlesExplicit k φ

/-- Nandao condition (iii) = presupposition of `kernelMust`. -/
theorem nandao_must_presupposition (k : Kernel) (φ : (World → Prop)) (w : World) :
...
theorem nandao_pure_inquiry (k : Kernel) (u : Background) (φ : (World → Prop))
    [DecidablePred φ]
