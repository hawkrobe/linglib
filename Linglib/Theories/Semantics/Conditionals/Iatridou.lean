...
**Kratzer-level counterpart**: In Kratzer's modal semantics, the same
X-marking operation corresponds to ∗-revision of the modal base
(`Kratzer.XMarking.IsStarRevision`): the `expandedDomain` parameter
here maps to the widened accessible world set ∩f'(w) ⊇ ∩f(w).
See @cite{ferreira-2023} for the full 2×2 square of necessities
generated by X-marking on both modal base (Xf) and ordering source (Xg). -/
