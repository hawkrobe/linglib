...
/-- Nominal comparative (Wellwood eq. 35):
    ...
    `extract = themeOf` (measure the theme entity), `role = agent`.
    The comparative measures the thematic object of the event:
    Al's coffee-amount exceeds Bill's coffee-amount. -/
...
/-- Verbal comparative (Wellwood eq. 43):
    ...
    `extract = id` (measure the event directly), `role = agent`.
    The comparative measures the event itself (by duration, distance, etc.). -/
...
/-- Adjectival comparative (Wellwood eq. 59):
    ...
    `extract = id` (measure the state directly), `role = holder`.
    The comparative measures the state variable: the hotness-state of
    this coffee exceeds the hotness-state of that coffee. -/
...
/-- Maximality reduction (Wellwood 2015, passim).
    ...
    truth condition with universal quantification over b's events
    reduces to direct comparison of measured values.

    This is the simplification used throughout Wellwood's derivations:
    the `-er` morpheme introduces a `max` operator, and when each
    individual's degree set is a singleton (unique maximal eventuality),
    `max({μ(extract(ea))}) > max({μ(extract(eb))})` reduces to
    `μ(extract(ea)) > μ(extract(eb))`. -/
...
/-- Adjectival comparative under maximality reduces to direct state
    ...
    When the state domain has a Preorder, this is exactly CSW's
    `statesComparativeSem μ sa sb` from StatesBased.lean (both are
    definitionally `μ sb < μ sa`). -/
...
/-- CSW's `statesComparativeSem` is definitionally the direct comparison
    `μ sb < μ sa`. This makes the identification between
    `adjectival_max_reduces` and CSW's formulation explicit. -/
...
/-- All comparative domains under maximality = comparativeSem (Rett/Schwarzschild).
    ...
    comparison as `comparativeSem` from Comparative.lean on measured values:
    `comparativeSem (μ ∘ extract) ea eb .positive = μ(extract(ea)) > μ(extract(eb))`.

    This unifies Wellwood's cross-categorial treatment with Rett's (2026)
    MAX-based framework: the MAX operator is trivial on singleton degree sets,
    and both frameworks yield the same truth conditions. -/
theorem max_eq_comparativeSem {Entity Time Measured : Type*} [LE Time]
    ...
    (hb_unique : ∀ e, role b e → P e → e = eb) :
    comparativeTruth role P extract μ a b ↔
      Semantics.Lexical.Adjective.Comparative.comparativeSem
...
/-- State domains are dimensionally restricted when linearly ordered:
    comparative truth conditions are independent of the choice of
    admissible measure function.

    This is the theoretical explanation for Wellwood's §3.4 observation:
    GAs measuring states (hot, hard) fix a single dimension because the
    state domain is linearly ordered, forcing all admissible measures
    to agree on the comparative ordering. -/
...
/-- If two admissible measures disagree on some pair, the domain is NOT
    ...
    This is the structural content of Wellwood's §3.4: entity/event
    domains have incomparable elements (e.g., a tall light object vs
    a short heavy object), so different measures (weight vs height)
    can yield different comparative orderings. -/
