...
def atom {Atom : Type} (a : Atom) : Individual Atom := {a}

/-- Construct a plural individual from a set of atoms. -/
def plural {Atom : Type} (s : Set Atom) : Individual Atom := s
...
1. Maps each world to the totality of instances (a set of atoms)
...
Since `Individual Atom = Set Atom`, this is `{ x | x ⊆ k.concept w }`.
...
theorem up_down_id (P : Property World Atom) (hMass : IsMass World Atom P) :
  ...
  intro k hk
  simp only [down, Option.some.injEq] at hk
  subst hk
  funext w; ext x
  simp only [up, Set.mem_setOf_eq]
  constructor
...
-/
theorem down_up_id (k : Kind World Atom) (hNat : k.isNatural) :
  ...
  congr 1
  cases k with
  | mk concept isNatural =>
    simp only at hNat
    have : True = isNatural := propext ⟨fun _ => hNat, fun _ => trivial⟩
    subst this; rfl
