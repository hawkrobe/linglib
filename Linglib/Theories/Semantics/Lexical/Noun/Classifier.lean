...
in `Theories.Semantics.Lexical.Plural.Link1983`.
...
namespace Semantics.Lexical.Noun.Classifier
...
def groupClf {E D : Type*} [SemilatticeSup E] [SemilatticeSup D]
    (mat : Semantics.Lexical.Plural.Link1983.Materialization E D)
    (P : E → Prop) (x : E) : Prop :=
...
/-- Dispatch from `ClassifierStrategy` to denotation function.

    - `.forNoun` → `clfForNoun P` (atomize the noun)
    - `.forNumeral` → `clfForNum P μ n` (measure-modify the numeral)

    This connects the typological enum in `Core.NounCategorization` to the
    semantic content defined above, making the dispatch structural rather
    than documentary. -/
...
end Semantics.Lexical.Noun.Classifier
