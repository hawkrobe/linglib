...
∪ (up) and ∩ (down) operators in `Semantics.Kinds.Chierchia1998`,
...
/-- Predicativize: extensional counterpart of Chierchia's ∪ (up) operator.
    ...
    The intensional generalization is `Semantics.Kinds.Chierchia1998.up`. -/
abbrev pred := @ident F
...
/-- NOM: Nominalization (@cite{partee-1987} Figure 1, @cite{chierchia-1984}).
    ...
    `Semantics.Kinds.Chierchia1998.down` (Chierchia's ∩). -/
...
/-- `NOM(pred(j)) = some j`: nominalizing the predicativization of an entity
    ...
    (`Semantics.Kinds.Chierchia1998.down_up_id`). -/
