...
/-- Round-trip identity: lowering a lifted DRS recovers the original.
    TODO: Prove by unfolding definitions and using set membership. -/
...
/-- ↑ is injective: distinct DRSs yield distinct state updates.
    TODO: Prove via round-trip. -/
theorem liftPW_injective (D₁ D₂ : DRS (Assignment E)) (h : liftPW (W := W) D₁ = liftPW D₂) :
    D₁ = D₂ := by
  sorry

/-- Lifted pointwise DRSs are always distributive (Charlow 2021, §6, key lemma).
    This is because `liftPW D` processes each element of the input state
    independently — it never "surveys" the entire context.
    TODO: Prove by showing the singleton decomposition. -/
