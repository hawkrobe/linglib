...
/-!
Utility non-monotonicity requires (Zaslavsky et al. 2020):
1. Graded lexicon: φ(m,u) ∈ (0,1], not binary {0,1}
2. α < 1: Below the rate-distortion critical point
...
No entry is exactly 0, enabling the dynamics to explore the full space.
-/
...
/--
Utility is not always monotone.
...
If not, the specific parameters do not trigger the phenomenon, but the
theoretical result still holds for appropriately constructed scenarios
(see Zaslavsky et al. Figure 2).
-/
...
The objective G_α = H_S + α·E[V_L] can increase even when E[V_L]
decreases, as long as H_S increases enough.
-/
...
- The speaker can increase G_α by making utterance choice more entropic
  (higher H_S), even if this decreases informativity (lower E[V_L])
...
At α = 1, G_α = -R[D] (negative rate-distortion function), the optimal
tradeoff between compression rate and distortion.
...
1. Higher-α speakers communicate better
2. There is a phase transition at α = 1
