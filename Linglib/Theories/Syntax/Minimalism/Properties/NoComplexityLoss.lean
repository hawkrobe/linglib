...
import Linglib.Theories.Syntax.Minimalism.Quotient
...
/-- Quotient tree has strictly lower degree when the extracted subtree
    ...
    From `quotientTree_leafCount`: q.leafCount = T.leafCount - v.leafCount + 1.
    With v.leafCount ≥ 2, this gives q.leafCount < T.leafCount. -/
...
theorem sideward_2b_violates_ncl (T₁ T₂ β q : SyntacticObject)
    (hc : contains T₁ β) (hq : quotientTree T₁ β = some q)
    (hv : β.leafCount ≥ 2)
