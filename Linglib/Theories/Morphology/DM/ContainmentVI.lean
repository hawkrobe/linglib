...
/-- Map a degree grade to its position index in the 3-cell hierarchy.
    Local helper; not added to `DegreeGrade`'s namespace because the
    only callers are inside this file. -/
private def gradePosition : DegreeGrade → Fin 3
  | .pos => 0
  | .cmpr => 1
  | .sprl => 2
...
def viPattern (rules : List LocalVIRule) (defaultForm : Nat) : DegreePattern :=
  ⟨viExponent (liftRules rules) defaultForm (gradePosition .pos),
   viExponent (liftRules rules) defaultForm (gradePosition .cmpr),
   viExponent (liftRules rules) defaultForm (gradePosition .sprl)⟩
