...
`Conditional.Marker`, and the language's X-marking exponent. *-ra* marks only hypothetical
...
(19) ([lassiter-2025]). -/
def ra : Conditional.Marker := ⟨"-ra", {.hypothetical}⟩
...
def nara : Conditional.Marker := ⟨"nara", {.hypothetical, .premise}⟩
...
conditionals (ex. 9a). -/
def eba : Conditional.Marker := ⟨"-(e)ba", {.hypothetical, .premise}⟩
