...
/-!
# K'iche' adposition order @cite{mondloch-2017} @cite{clemens-coon-2018}

K'iche' is not coded in WALS Ch 85 (ISO `quc` is absent), so the
classification is grammar-grounded from @cite{mondloch-2017} rather
...
`.noAdpositions` rather than `.prepositional`. @cite{clemens-coon-2018}
