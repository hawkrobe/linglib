...
This file provides the theoretical content that depends on `Verb.Root.FeatureSignature`:
...
def LevinClass.rootEntailments : LevinClass → Verb.Root.FeatureSignature
...
def _root_.Verb.Root.FeatureSignature.structuralMC (re : Verb.Root.FeatureSignature) : MeaningComponents :=
