...
theorem D_self_of_atom {P : E → Prop} {x : E} (hAtom : Atom x) (hP : P x) :
    D P x := by
  intro y hle hy
  rw [Atom.eq hAtom hle hy.not_null]
  exact hP
...
theorem pred_of_atom_star {P : E → Prop} :
  ...
  | @sum a b _ _ iha ihb =>
    by_cases ha : null a
    · rw [sup_eq_right.mpr (null_le ha)] at hAtom ⊢
      exact ihb hAtom
...
def AtomJoinPrime (E : Type*) [SemilatticeSup E] [Null E] : Prop :=
...
theorem distr_atom_part {P : E → Prop} (hDistr : IsDistr P)
  ...
  | base hp =>
    intro z hz hle
    have heq : z = _ := Atom.eq (hDistr _ hp) hle hz.not_null
    rwa [heq]
  | @sum a b _ _ iha ihb =>
    intro z hz hle
...
theorem distr_properPlural_extends {P : E → Prop}
  ...
  ⟨AlgClosure.sum (AlgClosure.base hx) (AlgClosure.base hy),
   fun hAtom => hne ((Atom.eq hAtom le_sup_left (hDistr x hx).not_null).trans
     (Atom.eq hAtom le_sup_right (hDistr y hy).not_null).symm)⟩
...
theorem properPlural_cum {P : E → Prop} {x y : E}
  ...
  by_cases hx0 : null x
  · rw [sup_eq_right.mpr (null_le hx0)] at hAtom
    exact hy.2 hAtom
