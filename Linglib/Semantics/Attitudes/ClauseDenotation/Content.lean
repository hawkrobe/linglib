...
3. Attitude verb nominalizations are NASNs, not ASNs, because the content
   individual is ∃-bound (not an external argument).
...
namespace Semantics.Attitudes.ClauseDenotation.Content
