...
* [K. J. Arrow, *A Difficulty in the Concept of Social Welfare* (1950)][arrow-1950]
...
/-- A profile assigns each object its vector of values, one per dimension. -/
abbrev Profile (ι α K : Type*) := α → ι → K

/-- An aggregation rule assigns to each profile a relation on the objects, read `x ⪰ y`. -/
...
/-- Pareto indifference says that objects with the same vector of values are indifferent. -/
...
/-- Anonymity says that permuting the dimensions leaves the output unchanged. -/
...
/-- Weak Pareto says that an object ranked strictly above another on every dimension is strictly
preferred. -/
def WeakPareto : Prop := ∀ v x y, (∀ i, v y i < v x i) → AsymmRel (a v) x y

/-- Strong Pareto says that an object ranked weakly above another on every dimension is weakly
...
/-- Dimension `i` is a dictator when its strict rankings are the strict overall rankings. -/
...
/-- Under majority rule `x ⪰ y` iff at least as many dimensions rank `x` weakly above `y` as rank
`y` weakly above `x`. -/
...
/-- Condorcet's profile has three dimensions ranking three objects cyclically. -/
def condorcet : Profile (Fin 3) (Fin 3) ℕ := ![![2, 0, 1], ![1, 2, 0], ![0, 1, 2]]

/-- Condorcet's paradox is that majority rule ranks the three objects in a strict cycle. -/
...
/-- Under the Pareto rule `x ⪰ y` iff every dimension ranks `x` weakly above `y`. -/
...
rescaling one dimension breaks a tie. -/
...
/-- `G` is decisive when the rule ranks `x` strictly above `y` whenever every dimension in `G` does.
-/
...
/-- `G` is almost decisive for the pair `x, y` when it prevails if every dimension outside `G` ranks
`y` strictly above `x`. -/
...
/-- By field expansion, with three or more objects a group almost decisive for one pair is decisive
for every pair. -/
...
/-- By group contraction, a decisive group with two or more dimensions has a decisive proper
subgroup. -/
...
/-- Arrow's theorem says that with three or more objects a rule that is ordinally invariant, outputs
