...
activity/result phases of a token). `Event.Phase` is type-level and modal —
...
* `Event.Phase` — precondition / occurrence / consequence over worlds.
* `Event.Phase.wellFormed` — occurrence entails precondition (the
  ontological constraint: you can't stop smoking unless you were smoking).
* `Event.Phase.isTelic` / `Event.Phase.isAtelic` — whether the consequence
