...
2. **Habitual type-shift** (γ, [cohen-2013]): semantically triggered
...
[cohen-2013] inherits the Partee-Rooth SHIFT operator to argue that
...
/-- Predicate Transfer for generics: T_g transforms a kind-level predicate
    ...
    [chierchia-1998], kinds are the denotations of bare plurals
...
/-- Partee-Rooth SHIFT: lift an extensional transitive verb to take
    ...
    [partee-rooth-1983] propose this to resolve the type mismatch
    when extensional verbs (type e → e → t) are conjoined with intensional
    verbs (type (et → t) → e → t): "John needed and bought a new coat."

    [cohen-2013] uses SHIFT to argue that type-shifting must be LOCAL:
...
/-- SHIFT does not commute with negation.
    ...
    This is the reason type-shifting is LOCAL ([cohen-2013] §13.3.1):
...
/-- The mechanism by which a covert quantifier is introduced into logical form.
    Determines scope behavior ([cohen-2013] §13.4). -/
