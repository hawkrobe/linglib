...
neither exhaustifier. The exhaustifier is `exh` with its alternative set selected by innocent
exclusion (`exhIE_eq_exh`).
...
theorem exhIE_eq_exh (hfin : ALT.Finite) :
  ...
  ext u
  rw [mem_exhIE_iff ALT φ hfin, mem_exh]
  refine and_congr_right λ hu => ⟨λ h a ha hua => absurd hua (h a ha), λ h a ha hua => ?_⟩
