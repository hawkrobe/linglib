...
A nonempty list generates its Hamblin issue, so under an antichain
condition (or the stronger pairwise disjointness of nonempty cells) the
alternatives of `ofList L` are exactly the listed elements. -/

/-- A Hamblin issue from a nonempty list is generated by the listed alternatives. -/
theorem coe_ofList {L : List (Set W)} (hL : L ≠ []) :
    (ofList L : Set (Set W)) = lowerClosure {p | p ∈ L} := by
  ext q
  simp only [SetLike.mem_coe, mem_ofList, mem_lowerClosure, Set.mem_ofPred_eq]
  refine ⟨?_, Or.inr⟩
  rintro (rfl | h)
  · obtain ⟨p, hp⟩ := List.exists_mem_of_ne_nil L hL
    exact ⟨p, hp, Set.empty_subset p⟩
  · exact h

/-- **Alternatives of `ofList`** under the antichain condition (no cell
    is contained in another): `alt (ofList L) = {p | p ∈ L}`. The
    hypothesis is weaker than pairwise disjointness — cells may overlap
    as long as no cell is a subset of any other distinct cell.
    ...
    Hamblin issue. -/
theorem alt_ofList_of_antichain (L : List (Set W)) (hL : L ≠ [])
    (hac : ∀ p₁ ∈ L, ∀ p₂ ∈ L, p₁ ≠ p₂ → ¬ p₁ ⊆ p₂) :
    alt (ofList L) = {p | p ∈ L} :=
  alt_eq_of_coe_eq_lowerClosure (fun p₁ h₁ p₂ h₂ ↦ hac p₁ h₁ p₂ h₂) (coe_ofList hL)

/-- **Alternatives of `ofList`** under pairwise disjoint + nonempty
    cells: special case of `alt_ofList_of_antichain`, since a
...
theorem alt_ofList_of_pairwise_disjoint_nonempty
    ...
    (hne : ∀ p ∈ L, p ≠ ∅) :
    alt (ofList L) = {p | p ∈ L} :=
  alt_ofList_of_antichain L hL
    fun p₁ h₁ p₂ h₂ h₁₂ hsub ↦ hne p₁ h₁ <| Set.subset_empty_iff.mp
      fun _w hw ↦ (hdisj p₁ h₁ p₂ h₂ h₁₂).le_bot ⟨hw, hsub hw⟩
