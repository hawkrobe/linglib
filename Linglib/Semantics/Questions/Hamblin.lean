...
Two basic question-content constructions, both built from `principal`
+ inquisitive disjunction:

- `polar p` — the polar interrogative `?p` for proposition `p`. Defined
  as `principal p ⊔ principal pᶜ`, matching [puncochar-2019]'s
...
- `which D P` — the wh-question content "which `e ∈ D` satisfies `P e`?",
  built as a Hamblin disjunction `⨆ e ∈ D, principal (P e)`. One
...
`info_principal`, and properties of the underlying `Set` operations.
-/
...
theorem polar_eq_sup (p : Set W) :
    polar p = principal p ⊔ principal pᶜ := rfl
...
@[simp] theorem mem_polar {p σ : Set W} : σ ∈ polar p ↔ σ ⊆ p ∨ σ ⊆ pᶜ := by
  rw [polar_eq_sup]; simp only [mem_sup, mem_principal]

@[simp] theorem info_polar (p : Set W) : (polar p).info = Set.univ := by
  rw [polar_eq_sup, info_sup, info_principal, info_principal,
...
/-- `declarative univ = ⊤`: the principal ideal of `univ` is the
    whole `LowerSet`. -/
theorem principal_univ : (principal (Set.univ : Set W)) = ⊤ := by
  ext q
...
theorem polar_empty : (polar (∅ : Set W)) = ⊤ := by
  rw [polar_eq_sup, Set.compl_empty, principal_univ, sup_top_eq]

/-- `polar univ = ⊤`: dual of `polar_empty`. -/
theorem polar_univ : (polar (Set.univ : Set W)) = ⊤ := by
  rw [polar_eq_sup, principal_univ, top_sup_eq]
...
theorem mem_ofList {L : List (Set W)} {q : Set W} :
  ...
  unfold ofList
  rw [mem_biSup_iff]
  simp only [Set.mem_setOf_eq, mem_principal]
