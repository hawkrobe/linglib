...
/-- An alternative of `which D P` is necessarily either the empty
    ...
    in any other `P e'`. -/
theorem eq_empty_or_exists_of_mem_alt_which {E : Type v} {D : Set E} {P : E → Set W} {q : Set W} :
    q ∈ alt (which D P) →
      (q = ∅ ∨
...
theorem alt_which_of_antichain {E : Type v} {D : Set E} {P : E → Set W}
  ...
  constructor
  · intro hq
    rcases eq_empty_or_exists_of_mem_alt_which hq with hempty | ⟨e, heD, rfl, _⟩
    · subst hempty
      obtain ⟨e₀, he₀⟩ := hD
      obtain ⟨-, hmax⟩ := hq
...
/-- **Alternatives of `ofList`** under the antichain condition (no cell
    ...
    ∈ L}`. The hypothesis is weaker than pairwise disjointness — cells
...
theorem alt_ofList_of_pairwise_disjoint_nonempty
  ...
  alt_ofList_of_antichain_nonempty L hL
    (fun p₁ h₁ p₂ h₂ h₁₂ hsub => hne p₁ h₁ <| Set.subset_empty_iff.mp
      fun _w hw => (hdisj p₁ h₁ p₂ h₂ h₁₂).le_bot ⟨hw, hsub hw⟩) hne
