...
Resolutions Theorem (`IsNormal.eq_iSup_ofSet_alt`) establishes for every
normal question. -/
...
/-- A partition issue is normal: a resolving state lies in a cell, and
the empty state lies in any cell, or is the only state when `W` is
empty. -/
theorem isNormal_fromSetoid (r : Setoid W) : (fromSetoid r).IsNormal := by
  rintro s (rfl | ⟨c, hc, hsc⟩)
  · rcases isEmpty_or_nonempty W with _ | ⟨⟨w⟩⟩
    · exact ⟨∅, ⟨Or.inl rfl, fun q _ _ ↦ (Set.eq_empty_of_isEmpty q).symm⟩, subset_rfl⟩
    · exact ⟨_, mem_alt_fromSetoid_of_mem_classes r (r.mem_classes w), Set.empty_subset _⟩
  · exact ⟨c, mem_alt_fromSetoid_of_mem_classes r hc, hsc⟩
...
/-- **Round-trip**, `Question → Setoid → Question`: a normal partition
issue is recovered from its setoid. Normality is not automatic: on `ℕ`,
adding the finite states to the issue of the parity partition leaves its
alternatives, the two cells, unchanged. -/
theorem fromSetoid_toSetoid {P : Question W} (h : P.IsPartition) (hN : P.IsNormal) :
    fromSetoid (toSetoid h) = P := by
  rw [fromSetoid_eq_iSup_ofSet_classes, classes_toSetoid]
  exact hN.eq_iSup_ofSet_alt.symm
