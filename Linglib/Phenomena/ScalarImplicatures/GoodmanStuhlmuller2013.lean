...
/-! ## Kennedy (2015) meets Goodman & Stuhlmuller (2013)

Kennedy's de-Fregean semantics assigns bare numerals an exact meaning:
"two" is true iff max count = 2. Kennedy (2015, §3.1) also derives a
lower-bound meaning via Partee's (1987) type-shifting (BE + iota + existential
closure). Both meanings are grammatically available.

We model this as interpretation ambiguity (cf. Scontras & Pearl 2021 for scope):
the listener marginalizes over both the speaker's observation (epistemic state)
and the interpretation (exact vs lower-bound). `Latent = Obs × KennedyInterp`.
