...
Minimal Word definitions for use in Phenomena data files.

These are simple Word values (surface form + category + features) for
constructing example sentences in empirical data descriptions.

## Design Rationale

This file provides theory-neutral lexical items. For richer lexical entries
with semantic properties (monotonicity, thresholds, etc.), see:
- `Fragments/English/Determiners.lean` - quantifiers with GQT/PT semantics
- `Fragments/English/Nouns.lean` - nouns with kind/bare plural licensing
- `Theories/Montague/Lexicon/` - compositional semantic denotations

Phenomena files should remain descriptive/theory-neutral, so they import from
here rather than Fragments (which imports from Theories).
-/
