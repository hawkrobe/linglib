...
/-! Note on imports: Mandarin Fragment files (`Mandarin/Predicates`,
`Mandarin/Particles`) are NOT imported here, even though the paper engages
Mandarin centrally. Reason: `Mandarin/Predicates` transitively pulls in
`Theories/Semantics/Verb/VerbEntry` → `Probabilistic.Measurement.Basic`,
which adds substantial weight (and at the time of writing is in a
mid-flight refactor by another session). The Mandarin-side analytical
content lives via the substrate (`AspHead`) rather than via Fragment
imports; the Mandarin drift sentry analogous to §9 below is
`Fragments.Mandarin.Predicates.liuyip_partition`, landed in
0.230.891 alongside the Mandarin verbs themselves. -/
...
- *No* new substrate files. Per the audit on this paper's integration plan,
  the Wurmbrand-Lohninger ICH, the Wurmbrand 2001 truncation operator, and
  the Pesetsky 2021 Exfoliation primitive all violate the ≥ 2-paper-anchor
  graduation rule. They live as local definitions here, ready to graduate
  when a second study consumes them.

- The substrate-level addition Liu & Yip's analysis *does* motivate is the
  bipartite split-aspect typing: `AspFlavor` and `AspHead`, landed in
  ...
  it is consumed by Travis 2010, MacDonald 2008, Tsai 2008, Sybesma 2017,
  and Liu & Yip 2026 jointly — so it warranted substrate placement.

## What this Studies file does NOT commit to

Per the audits:

- **Defective intervention is NOT bilateral labeling.** Liu & Yip's intervener
  is a *featurally-matching head occupying a probe position* (Chomsky 2000
  defective intervention), not a *category in a bilateral label*
  (Keine 2020 horizon opacity). The two coincide on the Type II / Type III
  contrast but make distinct predictions on featural mismatch. The
  `defectiveIntervention` predicate below uses head-as-intervener; it does
  *not* call `ProbeProfile.transparentToLabel`.
...
- **Wurmbrand-Lohninger ICH is not a baked-in `LinearOrder`** with an
  implication on transparency. The order on `ComplementClass` is independent
  of the claim that transparency is downward-closed; the latter is a
  theorem about a transparency relation, not a definitional property.
...
composition), `Fragments/Italian/Modals.lean`'s Hacquard 2006 restructuring
substrate, `Phenomena/Control/Studies/Landau2015.lean`'s `ControlTier`,
and `Theories/Syntax/Minimalist/Phase.lean`. The Cinque 2006 vs.
Wurmbrand 2001 restructuring rivalry is made explicit in §10 as a
refutation theorem candidate.
-/

namespace Phenomena.Complementation.Studies.LiuYip2026
...
/-- The Implicational Complementation Hierarchy of @cite{wurmbrand-lohninger-2023}:
    ...
    `LinearOrder` is *not* derived: the audit flagged that the implicational
    content of the ICH is a theorem about a *transparency relation*, not a
    structural property of the class lattice. The order here is just the
    enum's natural one (event < situation < proposition); the implicational
    claim is `transparency_downward_closed` below. -/
...
/-- Project a `ComplementSize` onto the Wurmbrand-Lohninger 3-tier
    `ComplementClass`, by `fValue` thresholds. This is Liu & Yip 2026's
    *Chinese-specific* mapping (the paper's footnote 22 explicitly notes
    that other languages may calibrate differently). The Studies file
    instantiates the mapping; a richer cross-linguistic substrate would
...
/-- Wurmbrand 2001-style restructuring: drop the topmost projected head from a
    `ClauseSpine`. Returns `none` if the spine has only one head (the floor).

    Local to this Studies file. The audit flagged this as failing the
    ≥2-consumer rule for substrate; promotion candidate when
    `Fragments/Italian/Modals.lean`'s informal restructuring discussion gets
    a Studies file or when a Wurmbrand 2014 study lands.
...
/-- Liu & Yip 2026's defective intervention (Chomsky 2000): an *embedded* head
    of the same category as the matrix probe blocks Agree, regardless of
    bilateral labeling. The featural-compatibility check captures the audit's
    insistence that intervention is by an *element occupying an embedded
    ...
    This predicate deliberately does NOT call `ProbeProfile.transparentToLabel`;
    the syntax audit on this paper's plan explained why the two mechanisms
    diverge. -/
...
/-- Studies-side projection: Mandarin *you* 'again' is typed as an
    ...
    is Liu & Yip's analytical commitment. -/
...
/-- Studies-side projection: Cantonese *-faan* 'again' is AspP_outer-associated
    but, unlike Mandarin *you*, does NOT carry a [+D] selectional restriction
    (it is compatible with stative *jau* 'have' per Liu & Yip §6 footnote on
    (78)). -/
...
/-- The audit-flagged contrast: Mandarin *you* and Cantonese *-faan* are
    BOTH outer-aspect, but only *you* carries [+D]. Encoding *-faan* with
...
/-- **Generalization I** (Liu & Yip 2026 (16)): in Mandarin, an *again*-element
    exhibits exceptional scopal behavior IFF it is outer-aspect-associated.
...
/-- **Generalization II** (Liu & Yip 2026 (20)): the exceptional scopal
...
/-- **Correlation I** (Liu & Yip 2026 (30), Mandarin): an *again*-element
...
/-- **Correlation II** (Liu & Yip 2026 (36)): an *again*-element exhibits
...
/-! Per the syntax audit: the minimal-vP claim is an *empirical* generalization
about Chinese, not a structural property derived from substrate. Stated here
as a per-fragment assertion that every nonfinite-clause-taking predicate in
the Mandarin and Cantonese fragments selects a complement of size ≥ vP.
Falsifiable by a single new datum. -/

/-! The Mandarin counterpart of the per-fragment minimal-vP sentry lives
    in `Fragments.Mandarin.Predicates.liuyip_partition` (commit 0.230.891).
    It records that the six Mandarin nonfinite-takers (xiang, rang, quan,
    bi, dasuan, shefa) all have `complementType = .infinitival` —
    consistent with vP-as-floor and falsifiable by a single new datum.
    Stated separately to keep this Studies file independent of the
    `Theories.Semantics.Verb.VerbEntry` import chain. -/

open Fragments.Cantonese.Predicates in
/-- All Cantonese nonfinite-takers select `[.vP]` per the per-language
    fragment classification (Liu & Yip's (79)). -/
...
The cross-framework reconciler audit on this paper's integration plan
flagged a hidden incoherence: the spine substrate (`ClauseSpine` +
...
The sharp refutation theorem the cross-framework audit suggested —
`¬ (Wurmbrand.truncated.projects .Asp ↔ Cinque.full.projects .Asp)` —
is contingent on a Cinque-style "always project" formalization landing
in `Theories/Syntax/Minimalist/`. Until then, the rivalry is
documented in prose, not theorem. -/

/-- A trivial Cinque-flavored "restructuring" (identity) for comparison
    with the Wurmbrand-flavored `restructure` above. Cinque 2006's claim
    is that restructuring is *projection-marking*, not truncation;
...
Per the cross-framework reconciler audit on this paper's integration plan,
...
- **HPSG** (`Theories/Syntax/HPSG/Core/Basic.lean`): no clausal spine, no
  ...
  `passiveRule`, `dativeShiftRule`, `resultativeRule`). Liu & Yip's
  movement+reconstruction has no HPSG analog. SILENT DIVERGENCE.

- **Dependency Grammar** (`Phenomena/Complementation/Studies/Osborne2019Control.lean`):
...
- **`Fragments/Italian/Modals.lean`** has a developed restructuring
  substrate based on @cite{rizzi-1978} + Hacquard 2006 (event-relativity
  analysis). The Liu & Yip formalization here uses a Wurmbrand 2001
  truncation operator (§4 above). The two are direct rivals; integration
  with the Italian substrate would be a productive next step but is
  deferred (the Italian file currently has no Wurmbrand-truncation
  bridge of its own).

- **`Phenomena/Control/Studies/Landau2015.lean`** has `ControlTier`
  (predicative vs logophoric) cross-classifying CTPs along an axis the
  Wurmbrand-Lohninger ICH `ComplementClass` (event/situation/proposition)
  parallels. A bridge theorem `LiuYip.proposition → Landau.logophoric`
  is candidate future work; both Studies files currently formalize
  their own complement-class projections without sharing substrate.

- **`Theories/Syntax/Minimalist/Phase.lean`** has Chomsky-2000/2001 phase
  machinery (`PICStrength`). Liu & Yip's intervention is by AspP_outer,
  a *non-phase* head — defective intervention is a third locality
  regime alongside `PICStrength.{strong, weak, linearizationBound}`.
  The current formalization silently bypasses the `Phase` substrate.
  Resolution: either extend `PICStrength` with a "head-as-intervener"
  case, or add explicit prose noting the alternative locality model. -/
...
/-- The local `ComplementClass` projects to the existing theory-neutral
    ...
    projection over substrate that already serves Deal 2026, Landau 2015,
    Cristofaro 2013, and Noonan 2007 — the interconnection-density
    discipline CLAUDE.md describes.

    Note: the projection collapses Wurmbrand-Lohninger's three classes
    onto two surface patterns (`barePropositionalCP` for proposition,
    `abarInternalCP` for situation/event). The collapse is correct for
    Chinese but may not generalize. -/
...
/-! ### Audit-flagged deferrals

- **Pesetsky 2021 Exfoliation**: not formalized here. The `shuo`-CP puzzle
  Liu & Yip resolve via Exfoliation (CP layer peelable under A-dependency)
  is left as prose. Pesetsky 2021 is in `references.bib`; promotion to
  a substrate operator awaits a second consumer.

- **`Theories/Semantics/Again/` substrate**: the audit recommended a
  uniform *again*-presupposition substrate (von Stechow 1996 / Beck 2006 /
  Bale 2007 parameterization). Without it, every *again*-element's
  ...
  type-equates *you*'s repetitive reading with *zai*'s. Bib entries
  `von-stechow-1996` and `beck-2006` are present; the substrate
  module is deferred until a German-*wieder* or English-*again* study
  joins.

- **The repetitive-vs-restitutive distinction** (T. Xu 2012, 2016; Liu
  2021, 2024): explicitly bracketed by Liu & Yip 2026 as out of scope.
  The Mandarin `you` Fragment entry currently encodes only the repetitive
  reading; a `you_restitutive` companion entry would be a small addition
  if needed.
...
- **Wurmbrand 2014a restructuring data** (German / Romance): the local
  `restructure` operator is Wurmbrand-2001-flavored and tested only on
  Chinese. Wurmbrand 2014a's lexical / functional restructuring
  dichotomy is a richer typology this Studies file does not yet engage. -/

end Phenomena.Complementation.Studies.LiuYip2026
