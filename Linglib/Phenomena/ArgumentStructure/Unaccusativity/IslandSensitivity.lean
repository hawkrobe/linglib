...
[lu-pan-degen-2025] [storment-2026]
...
file is the formalizer's synthesis: [storment-2026] doesn't
discuss MoS islands, and [lu-pan-degen-2025] doesn't run a
...
[lu-pan-degen-2025]: wh-extraction *from within* the complement
...
[storment-2026]: A-movement *of the entire VP* containing the
...
- Sub-extraction ([lu-pan-degen-2025]): wh-movement out of the
  complement, blocked by a discourse-sourced backgroundedness island
- VP-smuggling ([storment-2026]): A-movement of the whole VP,
...
`mosIslandSources` is from [lu-pan-degen-2025] (`Phenomena/Islands/
MannerOfSpeaking.lean`); `derivedQI` is the syntactic prediction from
[storment-2026] (`Semantics/Lexical/VerbSmuggling.lean`).
For every MoS verb the Fragment lists as unaccusative, both observations
hold simultaneously. -/

/-- The MoS verbs that pass QI (per [storment-2026]) and
    participate in MoS islands (per [lu-pan-degen-2025]). -/
...
/-- [lu-pan-degen-2025]'s finding: MoS islands are discourse-
    sourced. -/
theorem mos_islands_discourse_sourced : mosIslandSources = [.discourse] := rfl

/-- [storment-2026]'s prediction: every MoS unaccusative licenses
...
independent of [lu-pan-degen-2025]'s direct experimental tests.
