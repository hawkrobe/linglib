...
formalized here are the textbook tests originating with Vendler 1957
(the four-class taxonomy) and Dowty 1979 (the syntactic-test battery).
Anchored on a documented empirical pattern (per CLAUDE.md anchoring
rule), not on a specific paper-replication.
