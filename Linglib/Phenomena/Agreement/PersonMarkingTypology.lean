...
categories (attested combinations of participants).  The two theoretically
possible but cross-linguistically unattested groups (1+1 "choral we" and
2+2 "present audience only") are excluded. -/
...
/-! ### Bridge 1: PersonCategory ↔ UD.Person

Cysouw's singular categories correspond directly to UD person features.
The group categories have no direct UD.Person equivalent — they require
person + number together. -/
...
def PersonCategory.toUDPerson : PersonCategory → Option UD.Person
  ...
  | .s2 => some .second
  | .s3 => some .third
  | _   => none  -- groups don't map to a single UD.Person
...
def PersonCategory.fromUDPerson : UD.Person → Option PersonCategory
  ...
  | .second => some .s2
  | .third  => some .s3
  | .zero   => none  -- impersonal has no Cysouw equivalent

/-- Round-trip: UD.Person → PersonCategory → UD.Person is identity
    for the three core persons. -/
...
/-! ### Bridge 2: Group categories ↔ UD.Person + UD.Number

Non-singular person categories can be decomposed into a UD person feature
plus number feature.  This is the traditional analysis that Cysouw argues
against (it conflates qualitative group composition with quantitative number),
but it's the mapping needed for interoperability with UD-annotated data. -/
...
def PersonCategory.toUDPersonNumber :
  ...
  | .s2       => some (.second, .Sing)
  | .s3       => some (.third, .Sing)
  | .minIncl  => some (.first, .Dual)    -- closest UD: 1st dual
  | .augIncl  => some (.first, .Plur)    -- 1st plural (inclusive)
  | .excl     => some (.first, .Plur)    -- 1st plural (exclusive) — UD conflates!
  | .secondGrp => some (.second, .Plur)
  | .thirdGrp  => some (.third, .Plur)

/-- UD conflates inclusive and exclusive under first person plural.
    This is exactly Cysouw's critique of the traditional person × number grid. -/
theorem ud_conflates_incl_excl :
    PersonCategory.toUDPersonNumber .augIncl =
    PersonCategory.toUDPersonNumber .excl := by rfl
