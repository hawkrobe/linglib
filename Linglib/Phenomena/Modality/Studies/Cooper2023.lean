...
§6.3, Def A9: TYPE_MC = ⟨Type_M, BType, ⟨PType_M⟩,...⟩_{M ∈ M}.
...
§6.4, (5)–(7). A witness of □T is a proof that T has
...
§6.4, (20): τ : Topos has bg : Type and fg : (bg → Type).
τ(s) = τ.fg(s). -/

/-- A topos: a dependent type mapping background situations to foreground types.
§6.4, (20): if τ : Topos, then τ has Bg : Type, fg : (Bg → Type).
...
plate) vs bouletic (children eat food they love). §6.4.
...
§6.4, (29a): the deontic topos makes eating necessary. -/
...
§6.4, (29b): the bouletic topos also makes eating necessary. -/
...
§6.5, (50b): sell(a,b,c) ⊑ buy(c,b,a) is a postulated
...
§6.5, p.257: "r is a's total information state, then a's
...
This is a standard characterization; does not give a full
...
§6.5, (41): "for any relabelling η of T,
s : believe(a, [T]_η)". -/
...
§6.5, (55): pov(T₁, T₂) — T₁ is a point of view on T₂.
...
§6.5, (57): a :_E T iff either a : T or for some b ε a, b : T. -/
...
§6.5, (58) revises the witness condition for believe to
...
§6.5, (52): separate entries for Hesperus and Phosphorus. -/
...
§6.5, (53): entries unified with identity constraint. -/
...
§6.5: "e : des(a, T) just in case T is a's desires in e." -/
...
theorem restricted_snow_nec_i : nec_i twoModalRestricted .snow := by
  ...
  | false =>
    exfalso
    -- bare `simp` retained: closes the goal by rewriting `hp` to `False` via
    -- the booleq → False step that `simp only` doesn't pick up alone
...
theorem groundhog_ne_woodchuck : ¬ groundhog.intEq woodchuck := by
  intro h
  simp only [IType.intEq, groundhog, woodchuck, IType.mk.injEq] at h
...
theorem empty_types_distinct : ¬ roundSquare.intEq evenPrimeGt2 := by
  intro h
  simp only [IType.intEq, roundSquare, evenPrimeGt2, IType.mk.injEq] at h
  exact absurd h.2 (by decide)
...
/-- The compositional and construction readings are truth-conditionally
equivalent: both inhabited iff the individual has the property. -/
...
theorem cnstrIsA_intensionally_distinct :
  ...
  intro h
  simp only [IType.intEq, IType.mk.injEq] at h
  exact absurd h.2 (by decide)
...
-- ============================================================================
-- § 9. Cross-Framework Comparison: Cooper Topoi vs Kratzer Modal Backgrounds
-- ============================================================================

/-! ## Cooper's `Topos` ↔ Kratzer's `⟨ModalBase, OrderingSource⟩`

Cooper §6.4 explicitly positions topoi as the TTR replacement for Kratzer's
conversational backgrounds: "topoi replace accessibility relations between
possible worlds." This section makes the replacement claim Lean-checkable
by constructing a faithful translation in both directions and proving
equivalence on the necessity/possibility operators.
...
This is the load-bearing comparison that previous prose-only deferrals
in the Modality docstring (§6.4 prose, "Topoi replace Kratzer's
conversational backgrounds...") promised but did not deliver. -/
...
/-- Kratzer-to-Cooper translation. A modal base + ordering source +
evaluation world + proposition induces a Cooper topos:
...
def kratzerToTopos (f : ModalBase W) (g : OrderingSource W) (w : W)
    (p : W → Prop) : Topos where
  Bg := {w' : W // w' ∈ bestWorlds f g w}
  fg := fun s => PLift (p s.val)
...
theorem ttr_kratzer_d_axiom_agree (f : ModalBase W) (g : OrderingSource W)
    (w : W) (p : W → Prop)
    (hne : Nonempty {w' : W // w' ∈ bestWorlds f g w}) :
    necessity f g p w → possibility f g p w := by
  rw [← kratzer_topos_nec_iff, ← kratzer_topos_poss_iff]
  exact (kratzerToTopos f g w p).nec_implies_poss hne
...
This refutes the casual reading that "TTR replaces Kratzer's modal
backgrounds with topoi" implies a different modal arithmetic. The
arithmetic is the same; the framework difference is whether modal data
lives in worlds (Kratzer) or situations (Cooper). -/
theorem ttr_kratzer_modal_arithmetic_agree
    (f : ModalBase W) (g : OrderingSource W) (w : W) (p : W → Prop) :
    ((kratzerToTopos f g w p).inducedNec ↔ necessity f g p w) ∧
    ((kratzerToTopos f g w p).inducedPoss ↔ possibility f g p w) :=
  ⟨kratzer_topos_nec_iff f g w p, kratzer_topos_poss_iff f g w p⟩
