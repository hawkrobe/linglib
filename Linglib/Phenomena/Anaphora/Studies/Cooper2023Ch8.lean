...
namespace Phenomena.Anaphora.Studies.Cooper2023.Ch8
...
/-- Localization: absorb a parametric property's background into its body.
    Ch8 §8.5: 𝔏(P) moves context material into the property's domain type,
    enabling donkey anaphora. Before 𝔏: restrictor is parametric
    (depends on which donkey). After 𝔏: restrictor is pure (donkey
    existentially bound inside).

    Cooper §8.5 identifies 𝔏 with the Ch. 7 purification operator 𝔓
    (the same Σ-absorption of a parametric background into the
    ...
    about localization — the cross-chapter unification is true by
...
𝔏 and 𝔏ʸ. **Cooper's own §8.5 solution is 𝔓^∀ over a parametric property
whose background contains the ownership constraint** (not 𝔏 + a separate
gate). The current `farmerOwnsBeatsDonkey` puts ownership in the
foreground, so 𝔓^∀ over it misfires — but restructuring the property
so that `Bg` carries the owning relation (a per-farmer indexed
background) lets 𝔓^∀ produce the correct prediction directly.

Audit-verified gap (linguistics-domain-expert vs Cooper 2023 PDF, §8.5):
the correct strong-donkey reconstruction is deferred until the
indexed-background machinery for per-entity Bg is in place. -/
...
theorem boundary_clears_for_nonlocal :
    ({ localCtx₈ with 𝔩 := Core.PartialAssign.empty } : Cntxt₈ NLPInd).𝔩 0 = none := rfl
...
end Phenomena.Anaphora.Studies.Cooper2023.Ch8
