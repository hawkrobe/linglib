...
namespace Phenomena.Anaphora.Studies.Charlow2014
...
def cdrt : AnaphoraFramework farmer donkey owns beats where
  ...
  donkeyHolds := ∀ i : Register E,
    DProp.true_at (Phenomena.Anaphora.Studies.Cooper2023.cdrt_full_donkey
  ...
  donkey_iff_classical := by
    ...
    · intro h x y hf hd ho
      have := h (fun _ => x)
      rw [Phenomena.Anaphora.Studies.Cooper2023.full_donkey_equiv] at this
      exact ((this.some) x hf y hd ho).down
    · intro h i
      rw [Phenomena.Anaphora.Studies.Cooper2023.full_donkey_equiv]
...
def ttr : AnaphoraFramework farmer donkey owns beats where
  ...
  donkeyHolds := Nonempty (Phenomena.Anaphora.Studies.Cooper2023.ttr_full_donkey
                            farmer donkey owns beats)
  donkey_iff_classical := by
    unfold classicalDonkey Phenomena.Anaphora.Studies.Cooper2023.ttr_full_donkey
...
end Phenomena.Anaphora.Studies.Charlow2014
