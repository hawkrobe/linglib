...
namespace Phenomena.Verum.Studies.MartinezVera2026
...
theorem mi_polar_iff_verumFelicitous
    ...
    miFelicitous c ({{ w | β.atIssue w }, ({ w | β.atIssue w } : Set W)ᶜ}) β ↔
      Phenomena.Verum.Studies.Hohle1992.verumFelicitous c β := by
  constructor
...
theorem mi_eq_hohle_as_verumOperator
    (c : HighlightingContext W) (β : BiLayered W) (hne : ∃ w, β.atIssue w) :
    miAsVerumOperatorPolar.felicitous c β ↔
      Phenomena.Verum.Studies.Hohle1992.hohleAsVerumOperator.felicitous c β := by
  simp only [miAsVerumOperatorPolar_apply,
    Phenomena.Verum.Studies.Hohle1992.hohleAsVerumOperator_apply]
...
end Phenomena.Verum.Studies.MartinezVera2026
