...
/-!
# Numeral Semantics Bridge for Scontras & Pearl (2021) @cite{scontras-pearl-2021} @cite{kennedy-2015} @cite{musolino-2004}
...
1. **Kennedy (2015)**: de-Fregean semantics where bare numerals mean =n
2. **Musolino (2004)**: acquisition data — children reject "two" at w=3
...
internal/external distinction). Kennedy (2015, §3.1) resolves this:
exact is basic, lower-bound is derived via type-shift (Partee 1987).
...
/-- Kennedy's resolution: exact meaning is basic, lower-bound is derived
    via type-shift (Partee 1987). Both meanings are grammatically available.
