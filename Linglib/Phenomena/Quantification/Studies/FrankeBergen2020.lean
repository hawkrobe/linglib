...
practice; its alternatives {none, not-all} have negligible effect (footnote 7).
...
/-- Whether a meaning (World → Bool) is satisfiable. -/
def isSat (f : World → Bool) : Bool :=
  allWorlds.any f

/-- Pointwise entailment: f ⊨ g iff ∀ w, f(w) → g(w). -/
def entails (f g : World → Bool) : Bool :=
  allWorlds.all fun w => !f w || g w

/-- All sublists (subsets) of a list, as lists. -/
def sublists : List Nat → List (List Nat)
  | [] => [[]]
  | a :: as => let ss := sublists as; (ss.map (a :: ·)) ++ ss

/-- Innocent Exclusion: find indices of alternatives that appear in every
    maximal consistent exclusion set. Returns indices into `alts`. -/
def ieIndices (prejacent : World → Bool) (alts : List (World → Bool)) : List Nat :=
  let indices := List.range alts.length
  -- Non-entailed alternative indices
  let nonEnt := indices.filter fun i =>
    match alts[i]? with
    | some alt => !entails prejacent alt
    | _ => false
  -- All subsets of non-entailed indices
  let subsets := sublists nonEnt
  -- Consistent subsets: prejacent ∧ ¬(all in subset) is satisfiable
  let consistent := subsets.filter fun s =>
    isSat fun w => prejacent w && s.all fun i =>
      match alts[i]? with
      | some alt => !alt w
      | _ => true
  -- Maximal consistent subsets
  let maximal := consistent.filter fun s =>
    !consistent.any fun t =>
      s.length < t.length && s.all fun i => t.contains i
  -- IE = in every maximal set (or all if no maximal sets)
  if maximal.isEmpty then []
  else nonEnt.filter fun i => maximal.all fun m => m.contains i

/-- Apply Innocent Exclusion: keep prejacent, negate IE alternatives. -/
def applyIE (prejacent : World → Bool) (alts : List (World → Bool)) : World → Bool :=
  let ie := ieIndices prejacent alts
  fun w => prejacent w && ie.all fun i =>
    match alts[i]? with
    | some alt => !alt w
    | _ => true
...
inductive RSAModel where
  | vanilla
  | lexicalUncertainty   -- LU
  | lexicalIntentions     -- LI
  | globalIntentions      -- GI (best performer)
  deriving DecidableEq, BEq, Repr

/-- Posterior probability for each model (thousandths). -/
structure ModelResult where
  model : RSAModel
  posteriorProb : Nat
  deriving Repr

/-- Model comparison results: GI dominates. -/
def modelComparisonResults : List ModelResult :=
  [ ⟨.vanilla, 0⟩
  , ⟨.lexicalUncertainty, 10⟩
  , ⟨.lexicalIntentions, 33⟩
  , ⟨.globalIntentions, 956⟩ ]
...
/-- GI dominates all other models combined. -/
theorem gi_dominates :
    getPosterior .globalIntentions >
    getPosterior .vanilla + getPosterior .lexicalUncertainty +
    getPosterior .lexicalIntentions := by native_decide

-- ============================================================================
-- §9. Findings Summary
-- ============================================================================

/-- The 5 qualitative findings verified by this formalization. -/
...
noncomputable def formalize : Finding → Prop
  | .ss_inner_exh => giConfig.L1 .ss .wNS > giConfig.L1 .ss .wNSA
  | .ss_outer_exh => giConfig.L1 .ss .wNS > giConfig.L1 .ss .wS
  | .aa_identifies => giConfig.L1 .aa .wA > giConfig.L1 .aa .wSA
  | .as_inner_exh => giConfig.L1 .as_ .wS > giConfig.L1 .as_ .wA
  | .ss_parse_pref => giConfig.L1_latent .ss .moi > giConfig.L1_latent .ss .none

/-- All findings are verified by the GI-RSA model. -/
...
end Phenomena.Quantification.Studies.FrankeBergen2020
