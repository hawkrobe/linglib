...
that it is rational when the preorder is also connected.
...
* `Nonmonotonic.isPreferential_entails`, `Nonmonotonic.isRational_entails`: soundness for
  well-founded preorders and for well-founded connected preorders.
...
* The smoothness condition asks each definable set of states to have a minimal state below each
  ...
  order gives the condition for all of them (`Core.Order.Normality.exists_le_mem_optimal`). It
  holds on every finite set of worlds.
* A preorder is a term rather than an instance, as in `Core.Order.Normality`, because ordering
...
def Entails (p : Preorder W) (φ ψ : Set W) : Prop := optimal p φ ⊆ ψ

/-- When `ψ` is a consequence of `φ` in a well-founded model, the optimal worlds of `φ ∩ ψ` are
optimal for `φ`. -/
private theorem optimal_inter_subset (hp : WellFounded p.lt) (hψ : Entails p φ ψ) :
    optimal p (φ ∩ ψ) ⊆ optimal p φ := by
  intro w hw
  obtain ⟨v, hv, hvw⟩ := exists_le_mem_optimal hp hw.1.1
...
theorem isPreferential_entails (hp : WellFounded p.lt) : IsPreferential (Entails p) where
  refl φ := optimal_subset p φ
  rightWeakening h hψχ := h.trans hψχ
  and hψ hχ := Set.subset_inter hψ hχ
  or hφ hψ _ hw := hw.1.elim
    (fun h ↦ hφ (mem_optimal_of_subset Set.subset_union_left hw h))
    (fun h ↦ hψ (mem_optimal_of_subset Set.subset_union_right hw h))
  cautiousMonotonicity hψ hχ := (optimal_inter_subset hp hψ).trans hχ
...
/-- The consequence relation of a well-founded connected preorder is rational. An optimal
`φ`-world in `ψ` is as normal as every optimal `φ ∩ ψ`-world, so these are optimal for `φ`. -/
theorem isRational_entails (hp : WellFounded p.lt) (hc : connected p) :
  ...
  rationalMonotonicity := by
    ...
    have hwu : p.le w u := (hc w u).elim id (hw.2 ⟨hu.1, not_not.1 huψ⟩)
    exact hχ ⟨hw.1.1, fun v hv hvw ↦ p.le_trans _ _ _ hwu (hu.2 hv (p.le_trans _ _ _ hvw hwu))⟩

/-- Connectedness cannot be dropped. Let `0` be more normal than `2` and `1` comparable to
neither. Then normally `0` or `1` holds and it is not normal that `0` holds, yet given `1` or
`2` it is not normal that `0` or `1` holds, since `2` is then optimal. -/
theorem exists_not_isRational_entails : ∃ p : Preorder (Fin 3), ¬ IsRational (Entails p) := by
  ...
  have h₁ : Entails p Set.univ {0, 1} := fun w hw ↦ by
    ...
    clear hw
    revert w; decide
  have h1 : (1 : Fin 3) ∈ optimal p Set.univ :=
    ⟨trivial, fun v _ hv ↦ by revert hv; simp only [hle]; revert v; decide⟩
  have h2 : (2 : Fin 3) ∈ optimal p (Set.univ ∩ {1, 2}) :=
