...
This is the OT counterpart of the framework-neutral `otSystem`/`hgSystem`/
`maxEntSystem` constructors in `Constraints.System`, kept here so that the
neutral `Constraint` layer stays independent of the OT tableau machinery.
