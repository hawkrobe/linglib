...
open Core.Optimization.Evaluation in
...
instance (m : EvalMode) (a b : List Nat) : Decidable (le m a b) := by
  cases m with
  | parallel => exact (inferInstance : Decidable (Core.Optimization.Evaluation.LexLE a b))
  | directional dir =>
    cases dir with
    | leftToRight =>
      exact (inferInstance : Decidable (Core.Optimization.Evaluation.LexLE a b))
    | rightToLeft =>
