...
-- TODO(equivalence): package `Functor.EssSurj normalForm` (restricted to
-- the finite-vertex subcategory on both sides) with `normalForm.fullyFaithful`
-- into an `IsEquivalence` instance — the "packaged equivalence" side. The
-- `Full`, `Faithful` instances above transfer to the restriction automatically.

end EssSurj

/-! ### The monoidal upgrade

`isNormal` is preserved by `MultiAR.concat` (an empty orientation-bucket
stays empty under a union with the shifted image of another empty bucket)
and holds for `MultiAR.empty` (no links at all), so it is
`ObjectProperty.IsMonoidal`. This lifts the underlying `MultiAR.concat`
monoidal structure to `MultiAR.Normal` for free via mathlib's
`fullMonoidalSubcategory` machinery, with associator/left-unitor/right-unitor
still `eqToIso` of the strict monoid laws (`mul_assoc`, `one_mul`, `mul_one`
on `MultiAR τ`) — **Mac Lane coherence realized concretely as strict
equality on tuple concatenation, on the normal-form side of the
equivalence**. -/

section MonoidalUpgrade

/-- `MultiAR.empty` satisfies `isNormal` — with all link buckets empty, the
    excluded-orientation predicate holds vacuously. -/
theorem isNormal_empty : isNormal (MultiAR.empty (τ := τ)) :=
  fun _ _ _ => rfl

/-- `isNormal` is preserved by `MultiAR.concat`: each excluded-orientation
    bucket in the concatenation is the union of the two factors' excluded
    buckets (shifted), each empty by hypothesis. -/
theorem isNormal_concat {A B : MultiAR τ} (hA : isNormal A) (hB : isNormal B) :
    isNormal (A.concat B) := by
  intro i j hle
  rw [MultiAR.links_concat, hA i j hle, hB i j hle]
  simp

instance : ObjectProperty.ContainsUnit (isNormal (τ := τ)) := ⟨isNormal_empty⟩

instance : ObjectProperty.TensorLE (isNormal (τ := τ)) isNormal isNormal :=
  ⟨fun _ _ => isNormal_concat⟩

instance : ObjectProperty.IsMonoidal (isNormal (τ := τ)) where

/-- **Strictness payoff.** On normal forms, tuple concatenation is
    associative on the nose — the associator in `MultiAR.Normal`'s monoidal
    structure is `eqToIso (mul_assoc ..)`, an equality-derived isomorphism,
    not a nontrivial coherence morphism. This equality is the underlying
    content of `α_ A B C` in `MultiAR.Normal`. -/
theorem MultiAR.Normal.concat_assoc (A B C : MultiAR.Normal (τ := τ)) :
    (A.obj.concat B.obj).concat C.obj = A.obj.concat (B.obj.concat C.obj) :=
  mul_assoc A.obj B.obj C.obj

/-- Left unit law on normal forms — strictly. -/
theorem MultiAR.Normal.empty_concat (A : MultiAR.Normal (τ := τ)) :
    (MultiAR.empty : MultiAR τ).concat A.obj = A.obj :=
  one_mul A.obj

/-- Right unit law on normal forms — strictly. -/
theorem MultiAR.Normal.concat_empty (A : MultiAR.Normal (τ := τ)) :
    A.obj.concat (MultiAR.empty : MultiAR τ) = A.obj :=
  mul_one A.obj

-- TODO(monoidal-functor): upgrade `normalForm` to `Functor.LaxMonoidal` /
-- `Functor.Monoidal` by providing the comparison isos
-- `normalForm.obj X ⊗ normalForm.obj Y ≅ normalForm.obj (X ⊗ Y)` on the
-- foundation side. Requires the concrete equivalence
-- `(A.concat B).toMultiGraph.toMixedGraphCat.V ≃ ((A ⊗ B) : Rep).obj.V`,
-- factoring through `finSumFinEquiv.symm.sigma` — mechanical but not
-- one-liner; leaves the sigma-of-sum-of-Fin bookkeeping to the next stage
-- (paired with the dissolution rename to `AR`/`Graph`).
