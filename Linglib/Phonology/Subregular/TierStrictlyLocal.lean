...
* `TierStrictlyLocalGrammar k α`: a decidable tier together with an `SL_k` grammar read on the
  projection.
* `IsTierStrictlyLocal k L`: `L` is generated by some `TierStrictlyLocalGrammar k α`.
...
attribute [instance] TierStrictlyLocalGrammar.decTier

/-- The language generated by a TSL grammar: the preimage, under tier
projection, of the `SL_k` language of `permitted`. -/
def language (G : TierStrictlyLocalGrammar k α) : Language α :=
  tierProject G.tier ⁻¹' G.permitted.language k

@[simp] lemma mem_language (G : TierStrictlyLocalGrammar k α) (w : List α) :
    w ∈ G.language ↔
...
`TierStrictlyLocalGrammar k α` generates it. -/
def IsTierStrictlyLocal (k : ℕ) (L : Language α) : Prop :=
  ∃ G : Subregular.TierStrictlyLocalGrammar k α, G.language = L

end Language

namespace Subregular

/-- The language of a `TierStrictlyLocalGrammar` is tier-based strictly local: the
introduction rule for `Language.IsTierStrictlyLocal`. -/
theorem TierStrictlyLocalGrammar.isTierStrictlyLocal_language {k : ℕ}
    (G : TierStrictlyLocalGrammar k α) :
    Language.IsTierStrictlyLocal k G.language := ⟨G, rfl⟩
...
theorem IsTierStrictlyLocal.isStarFree {k : ℕ} {L : Language α} (h : IsTierStrictlyLocal k L) :
  ...
  rwa [show {w : List α | tierProjectHom G.tier (FreeMonoid.ofList w)
      ∈ G.permitted.language k} = G.language from rfl] at this
