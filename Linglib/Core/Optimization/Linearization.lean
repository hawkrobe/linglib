...
/-- A coordinate-wise strict inequality admits a strictly positive weighting whose weighted sums
order the vectors strictly, namely weight `∑ k, f k + 1` on the strict coordinate and `1` elsewhere.
-/
...
/-- Pareto-incomparable vectors are ordered oppositely by two strictly positive weightings, so
weighted-sum models can break the tie where Pareto reasoning cannot. -/
