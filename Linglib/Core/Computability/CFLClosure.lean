...
/-- The homomorphic image of a language under a string homomorphism.
    ...
    Given `h : StringHom α β` (a letterwise free-monoid homomorphism, see
    `Core.StringHom`) and a language `L : Language α`, this is the language
    `{ h(v) | v ∈ L }` over the target alphabet `β`.

    Definitionally equivalent to mathlib's letter-to-letter `Language.map`
    when `h` is `letterMap f`, but `StringHom` is strictly more general
    (each input letter can map to a multi-letter output, including the
    empty string). -/
def Language.stringMap {α : Type u} {β : Type v}
    (h : StringHom α β) (L : Language α) : Language β :=
  {w | ∃ v ∈ L, StringHom.apply h v = w}
...
/-- **CFL closure under string homomorphism** (@cite{hopcroft-ullman-1979}
    ...
    module docstring of `Linglib/Core/Computability/CFLClosure.lean`. -/
axiom Language.IsContextFree.stringMap {α β : Type}
    (h : StringHom α β) {L : Language α}
    (hL : L.IsContextFree) :
...
/-- **CFL closure under intersection with a regular language**
    ...
    see the module docstring of `Linglib/Core/Computability/CFLClosure.lean`. -/
