...
[UPSTREAM] candidate: `Mathlib.Computability.SyntacticCon`. Group theory has no name for this
construction — the analogue for a subgroup is `Subgroup.normalCore`, with no monoid counterpart —
and "syntactic" is formal-language vocabulary, so it belongs with the language theory rather than
in an algebra directory.
-/
