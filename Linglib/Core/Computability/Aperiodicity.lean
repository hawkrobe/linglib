...
`none` is absorbing; otherwise a forbidden subsequence completes — some recorded member
extended by `a` is not in `G` — and we die, else we record every new bounded subsequence
ending in `a`. -/
private noncomputable def step : Option (Profile α n) → α → Option (Profile α n)
  | none, _ => none
  | some P, a =>
    if ∃ s ∈ P, s.val ++ [a] ∉ G then none
...
private lemma evalFrom_eq_none_iff (xs ys : List α) (hxs : ¬ Bad G n xs) :
  ...
  | cons a ys ih =>
    ...
    split
    · rename_i hdie
      obtain ⟨s, hs, hbad⟩ := hdie
      simp only [evalFrom_none, true_iff]
      refine ⟨s.val ++ [a], ?_, by simpa using s.2, hbad⟩
      exact hs.append (List.singleton_sublist.mpr (List.mem_cons_self ..))
    · rename_i halive
      have hxsa : ¬ Bad G n (xs ++ [a]) := by
        rintro ⟨f, hf, hlen, hg⟩
        obtain ⟨f₀, hf₀sub, rfl⟩ := bad_snoc n xs a hf (fun hc => hxs ⟨f, hc, hlen, hg⟩) hlen
        exact halive ⟨f₀, by simpa [profOf] using hf₀sub, hg⟩
...
/-- Once `ε` is permitted the empty word has no forbidden subsequence, so the start profile is
clean. -/
private lemma not_bad_nil (h0 : ([] : List α) ∈ G) : ¬ Bad G n [] := by
  rintro ⟨f, hf, -, hg⟩
  rw [List.sublist_nil.mp hf] at hg
  exact hg h0
...
finite aperiodic recognizer ([heinz-rogers-2010] [mcnaughton-papert-1971]). Banning `ε`
already empties the language, so only the `ε ∈ G` case needs the scanner. -/
private theorem isStarFree_of_language_succ (L : Language α) [Finite α]
    (hL : G.language (n + 1) = L) : L.IsStarFree := by
  classical
