...
context-free. Construction: the **Bar-Hillel triple-product** (Bar-Hillel,
Perles & Shamir 1961; @cite{hopcroft-ullman-1979} Theorem 6.5).

Given a CFG `G = (V, T, P, S)` and a DFA `M = (Q, Σ, δ, q₀, F)` for `R`, build
a CFG `G'` with nonterminals `Q × V × Q ⊕ Unit` whose rules track DFA state
transitions:
...
/-- The product nonterminal type: state-pair-annotated G-nonterminals plus a
    fresh start symbol `inr ()`. -/
abbrev productNT (G : ContextFreeGrammar T) (σ : Type) : Type := σ × G.NT × σ ⊕ Unit
...
theorem product_language_subset (G : ContextFreeGrammar T) (M : DFA T σ) :
