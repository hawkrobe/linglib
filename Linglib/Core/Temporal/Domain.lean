...
import Linglib.Core.Temporal.OrientationTime
...
- **NamedTO**: a TO together with a typed `Role` label. The `Role` is
  ...
  type-safe across frameworks: `OrientationTime` (the canonical
...
abbrev TO (Time : Type*) [LE Time] := Interval Time
...
/-- A TO with an identifying typed role label. The `Role` parameter is
    abstract: framework-specific role types (`OrientationTime` for
...
structure NamedTO (Time : Type*) (Role : Type*) [LE Time] where
  /-- The role label (e.g. `OrientationTime.perspective`, `.topic`). -/
  name : Role
  /-- The TO itself. -/
  span : TO Time
...
structure Domain (Time : Type*) (Role : Type*) [LE Time] where
...
/-- The Allen relation from one TO to another, lifted via the
    `Interval.allenRel` projection function. Noncomputable (uses
    `Classical.choose`). For non-degenerate TOs this is the unique
    atom holding between them. -/
noncomputable def allenRel (i j : TO Time) : AllenRelation :=
  Interval.allenRel i j
...
/-! Convenience constructors for the canonical Reichenbach role-set,
    using `OrientationTime` as the universal `Role` type:
...
/-- The four canonical Reichenbach role labels (extended with
    @cite{kiparsky-2002}'s perspective time P) as `OrientationTime`
    values: utterance (S), perspective (P), topic (R), situation (E). -/
def reichenbachLabels : List OrientationTime :=
...
def ofReichenbachPoints (S P R E : Time) : Domain Time OrientationTime where
