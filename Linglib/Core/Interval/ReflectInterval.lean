...
- `RExpr.eval_sound : ∀ e, (e.eval).containsReal e.denote` — soundness

The `rsa_decide` tactic reifies ℝ expressions into `RExpr` values, then:
1. `native_decide` evaluates `(rhs.eval).hi < (lhs.eval).lo` in compiled code
2. The kernel verifies only two `eval_sound` applications + one `native_decide`
...
def RExpr.eval : RExpr → QInterval
  ...
  | .div a b =>
    ...
    if ra.lo == 0 && ra.hi == 0 then QInterval.exact 0
      ...
      else ⟨-1, 1, by norm_num⟩  -- fallback (shouldn't happen in RSA)
...
Every `RExpr` evaluates to a `QInterval` that contains the real denotation. -/
theorem RExpr.eval_sound : ∀ (e : RExpr), (e.eval).containsReal e.denote
  | .nat n => by
    simp only [eval, denote]
    exact QInterval.exact_containsReal n
  | .add a b => by
    simp only [eval, denote]
    exact QInterval.add_containsReal (eval_sound a) (eval_sound b)
  | .mul a b => by
    simp only [eval, denote]
    have iha := eval_sound a
    have ihb := eval_sound b
    split
    · -- ra.lo == 0 && ra.hi == 0
      rename_i h
  ...
  | .neg a => by
    simp only [eval, denote]
    exact QInterval.neg_containsReal (eval_sound a)
  | .sub a b => by
    simp only [eval, denote]
    exact QInterval.sub_containsReal (eval_sound a) (eval_sound b)
  | .rexp a => by
    simp only [eval, denote]
    exact expInterval_containsReal (eval_sound a)
  | .rlog a => by
    simp only [eval, denote]
    have iha := eval_sound a
    split
    · exact logInterval_containsReal ‹_› iha
    · sorry  -- fallback: log with non-positive lo (unreachable via iteZero guard)
  | .rpow a n => by
    simp only [eval, denote]
    have iha := eval_sound a
    split
  ...
  | .iteZero c t e => by
    simp only [eval, denote]
    have ihc := eval_sound c
    have iht := eval_sound t
    have ihe := eval_sound e
    split
...
theorem RExpr.gt_of_eval_separated (lhs rhs : RExpr)
  ...
  QInterval.gt_of_separated (eval_sound lhs) (eval_sound rhs) h
