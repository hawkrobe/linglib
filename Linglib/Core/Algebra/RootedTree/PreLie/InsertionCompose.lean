...
/-- The vertex multiset of a grafted tree: transported originals plus
    ...
    `pairSources`). -/
theorem vertices_multiGraft_transport_lift (T : RoseTree α)
...
private theorem multiGraft_single_pair (W : RoseTree α) (p : Path) (v : RoseTree α) :
...
theorem insertion_singleton_guest (W v : RoseTree α) :
    insertion W [v] =
      ((vertices W : List Path) : Multiset Path).map
        (fun p => insertAt p v W) := by
  rw [insertion_def,
      show listChoices (vertices W) ([v] : List (RoseTree α)).length =
          (vertices W).map (fun p => [p]) from by
        rw [show ([v] : List (RoseTree α)).length = 1 from rfl, listChoices_succ]
  ...
  rw [show ((((vertices W).map
        ...
        List (RoseTree α)) : Multiset (RoseTree α)) =
      ((vertices W : List Path) : Multiset Path).map
...
theorem insertion_multiGraft_singleton (T : RoseTree α)
    (pairs : List (Path × RoseTree α)) (v : RoseTree α)
    (h_valid : ∀ pair ∈ pairs, IsValidPath pair.fst T) :
...
theorem insertionForest_singleton_guest_set (B : List (RoseTree α))
    (v : RoseTree α) :
    insertionForest B [v] =
  ...
  induction B with
  | nil =>
    show insertionForest ([] : List (RoseTree α)) [v] = _
    rw [insertionForest_empty_host_nonempty_guests]
    show (0 : Multiset _) =
      (Multiset.ofList (List.finRange ([] : List (RoseTree α)).length)).bind _
    rfl
  | cons b B' ih =>
    rw [insertionForest_cons_assignment]
    -- listChoices [t,f] 1 = [[true],[false]]
    rw [show ([v] : List (RoseTree α)).length = 1 from rfl]
    ...
    rw [show insertion b ([] : List (RoseTree α)) = ({b} : Multiset (RoseTree α)) from by
          rw [insertion_def]
          show (Multiset.ofList [multiGraft b []] : Multiset (RoseTree α)) = {b}
          rw [multiGraft_nil]; rfl]
    rw [Multiset.singleton_bind]
    rw [show (fun T' => Multiset.map (fun F' => T' :: F') ({B'} : Multiset (List (RoseTree α)))) =
            (fun T' => ({T' :: B'} : Multiset (List (RoseTree α)))) from by
          funext T'; rfl]
...
theorem insertion_bind_singleton (T : RoseTree α) (B : List (RoseTree α))
    (v : RoseTree α) :
            ...
            fun k : Fin (c.zip B).length =>
              ((vertices (c.zip B)[k].snd : List Path) : Multiset Path).bind
                (fun q =>
                  ({multiGraft T
                    ((c.zip B).set k.val ((c.zip B)[k].fst,
                      insertAt q v (c.zip B)[k].snd))} : Multiset (RoseTree α)))) from by
        refine Multiset.bind_congr fun k _ => ?_
        rw [← Multiset.bind_singleton]]
...
private theorem insertionForest_cons_singleton (T : RoseTree α)
    (F : List (RoseTree α)) (v : RoseTree α) :
  ...
  rw [show (fun T' : RoseTree α =>
            ({F} : Multiset (List (RoseTree α))).map (fun F' => T' :: F')) =
          (fun T' : RoseTree α => ({T' :: F} : Multiset (List (RoseTree α)))) from by
        funext T'; rw [Multiset.map_singleton]]
  rw [show (fun T' : RoseTree α => ({T' :: F} : Multiset (List (RoseTree α)))) =
          (fun T' : RoseTree α => ({(fun T'' => T'' :: F) T'} : Multiset _)) from rfl]
...
theorem insertionForest_singleton_bucket_split
    {γ : Type*} (B : List (RoseTree α)) (m : List Bool) (v : RoseTree α)
    (G : List (RoseTree α) → List (RoseTree α) → Multiset γ)
...
theorem insertionForest_bind_singleton (A B : List (RoseTree α))
    (v : RoseTree α) :
  ...
  | nil =>
    ...
    | cons b B' =>
      ...
      rw [show (fun k : Fin (b :: B').length =>
            ...
            (fun _ : Fin (b :: B').length => (0 : Multiset (List (RoseTree α)))) from by
          funext k
          rw [Multiset.bind_map]
          -- Each inner: insertionForest [] (b::B').set k _ — nonempty because set preserves cons-shape
          -- Show: bind q ∈ vertices, 0 = 0 via bind_zero
          have set_cons_form : ∀ (xs : List (RoseTree α)) (n : ℕ) (w : RoseTree α),
              ∃ x' xs', ((b :: xs).set n w : List (RoseTree α)) = x' :: xs' := by
            intro xs n w
            cases n with
            | zero => exact ⟨w, xs, rfl⟩
          ...
          rw [show ((vertices (b :: B')[k] : List Path) : Multiset Path).bind
                (fun q => insertionForest ([] : List (RoseTree α))
                  ((b :: B').set k.val (insertAt q v (b :: B')[k]))) =
                  ((vertices (b :: B')[k] : List Path) : Multiset Path).bind
                  (fun _ => (0 : Multiset (List (RoseTree α)))) from by
                refine Multiset.bind_congr fun q _ => ?_
                exact this q]
