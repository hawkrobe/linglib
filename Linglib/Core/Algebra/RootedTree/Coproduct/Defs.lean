...
/-!
# Generalized cut-summand enumeration on `Tree α`
...
`extract : Tree α → Option (List (Tree α))`. A cut at a child
...
remainder type is `List (Tree α)` (zero, one, or many replacement
...
def cutSummandsG (extract : Tree α → Option (List (Tree α))) :
    Tree α → Multiset (Forest (Tree α) × Tree α)
...
def cutListSummandsG (extract : Tree α → Option (List (Tree α))) :
    List (Tree α) → Multiset (Forest (Tree α) × List (Tree α))
...
def augActionG (extract : Tree α → Option (List (Tree α))) :
    Tree α → Multiset (Forest (Tree α) × List (Tree α))
  | t =>
      (match extract t with
       | none => (0 : Multiset _)
       | some r => {(({t} : Forest (Tree α)), r)})
...
@[simp] theorem cutSummandsG_node
    (extract : Tree α → Option (List (Tree α)))
    (a : α) (cs : List (Tree α)) :
    cutSummandsG extract (Tree.node a cs) =
...
@[simp] theorem cutListSummandsG_nil
    (extract : Tree α → Option (List (Tree α))) :
    cutListSummandsG extract ([] : List (Tree α)) =
...
@[simp] theorem augActionG_eq
    (extract : Tree α → Option (List (Tree α))) (t : Tree α) :
...
theorem augActionG_eq_some
    (extract : Tree α → Option (List (Tree α))) (t : Tree α)
    (r : List (Tree α)) (h : extract t = some r) :
    augActionG extract t =
      (({t} : Forest (Tree α)), r) ::ₘ
        (cutSummandsG extract t).map (fun p => (p.1, [p.2])) := by
  rw [augActionG_eq, h]
...
A child list's total node count is `(l.map Tree.numNodes).sum`, so
`List.map_append`/`List.sum_append` discharge the append step directly
and `Tree.numNodes_node` unfolds the node count — no bespoke child-list
...
theorem cutSummandsG_weight
    (extract : Tree α → Option (List (Tree α)))
    (hext : ∀ t r, extract t = some r → (r.map Tree.numNodes).sum = 1) :
    ∀ (t : Tree α), ∀ p ∈ cutSummandsG extract t,
      (Multiset.map Tree.numNodes p.1).sum + Tree.numNodes p.2 =
        Tree.numNodes t + Multiset.card p.1
  | .node a cs => by
    intro p hp
    rw [cutSummandsG_node] at hp
    obtain ⟨q, hq, rfl⟩ := Multiset.mem_map.mp hp
    have h := cutListSummandsG_weight extract hext cs q hq
    simp only [Tree.numNodes_node]
    omega
...
theorem cutListSummandsG_weight
    (extract : Tree α → Option (List (Tree α)))
    (hext : ∀ t r, extract t = some r → (r.map Tree.numNodes).sum = 1) :
    ∀ (cs : List (Tree α)), ∀ q ∈ cutListSummandsG extract cs,
      (Multiset.map Tree.numNodes q.1).sum + (q.2.map Tree.numNodes).sum =
        (cs.map Tree.numNodes).sum + Multiset.card q.1
  | [] => by
    intro q hq
...
theorem augActionG_weight
    (extract : Tree α → Option (List (Tree α)))
    (hext : ∀ t r, extract t = some r → (r.map Tree.numNodes).sum = 1) :
    ∀ (t : Tree α), ∀ a ∈ augActionG extract t,
      (Multiset.map Tree.numNodes a.1).sum + (a.2.map Tree.numNodes).sum =
        Tree.numNodes t + Multiset.card a.1
  | t => by
    intro a ha
...
example (extract : Tree Unit → Option (List (Tree Unit))) :
    cutSummandsG extract (Tree.leaf () : Tree Unit)
      = {((0 : Forest (Tree Unit)), (Tree.leaf () : Tree Unit))} := by
  show cutSummandsG extract (Tree.node () []) = _
