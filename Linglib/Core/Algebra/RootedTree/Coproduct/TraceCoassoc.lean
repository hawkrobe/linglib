...
/-!
# Tree double-cut coassociativity for Δ^c (combinatorial core of MCB 1.2.10)
...
abbrev FP (α : Type*) := Forest (Tree α)
...
theorem forestCutsP_add (τ : Tree (α ⊕ β) → β) (F G : FP (α ⊕ β)) :
...
theorem hR_hom (τ : Tree (α ⊕ β) → β) (p q : Pair (α ⊕ β)) :
...
theorem dcLHSP_eq_bind (τ : Tree (α ⊕ β) → β) (t : Tree (α ⊕ β)) :
    dcLHSP τ t = (treeCutsP τ t).bind (hL τ) := rfl

theorem dcRHSP_eq_bind (τ : Tree (α ⊕ β) → β) (t : Tree (α ⊕ β)) :
    dcRHSP τ t = (treeCutsP τ t).bind (hR τ) := rfl

theorem dcForestLHSP_eq_bind (τ : Tree (α ⊕ β) → β) (F : FP (α ⊕ β)) :
    dcForestLHSP τ F = (forestCutsP τ F).bind (hL τ) := rfl

theorem dcForestRHSP_eq_bind (τ : Tree (α ⊕ β) → β) (F : FP (α ⊕ β)) :
    dcForestRHSP τ F = (forestCutsP τ F).bind (hR τ) := rfl

/-! ### Multiplicativity of the forest double-cut -/

theorem dcForestLHSP_cons (τ : Tree (α ⊕ β) → β)
    (t : Tree (α ⊕ β)) (F : FP (α ⊕ β)) :
...
theorem dcForestRHSP_cons (τ : Tree (α ⊕ β) → β)
    (t : Tree (α ⊕ β)) (F : FP (α ⊕ β)) :
...
theorem dcForestLHSP_zero (τ : Tree (α ⊕ β) → β) :
...
theorem dcForestRHSP_zero (τ : Tree (α ⊕ β) → β) :
...
theorem coassFP_of_components (τ : Tree (α ⊕ β) → β) (F : FP (α ⊕ β))
...
abbrev Cl (τ : Tree (α ⊕ β) → β) (cs : List (Tree (α ⊕ β))) :
    Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β))) :=
...
theorem treeCutsP_node (τ : Tree (α ⊕ β) → β) (a : α ⊕ β) (cs : List (Tree (α ⊕ β))) :
    treeCutsP τ (Tree.node a cs) =
      (({Tree.node a cs}, 0) : Pair (α ⊕ β)) ::ₘ
        (Cl τ cs).map (fun p => ((p.1, {Tree.node a p.2}) : Pair (α ⊕ β))) := by
  unfold treeCutsP
...
theorem dcLHSP_node (τ : Tree (α ⊕ β) → β) (a : α ⊕ β) (cs : List (Tree (α ⊕ β))) :
    dcLHSP τ (Tree.node a cs) =
      ((({Tree.node a cs} : FP (α ⊕ β)), (0 : FP (α ⊕ β)), (0 : FP (α ⊕ β))) ::ₘ
        (Cl τ cs).map (fun p => (p.1, ({Tree.node a p.2} : FP (α ⊕ β)), (0 : FP (α ⊕ β)))))
      + (Cl τ cs).bind (fun p =>
          (forestCutsP τ p.1).map
            (fun c12 => (c12.1, c12.2, ({Tree.node a p.2} : FP (α ⊕ β))))) := by
  rw [dcLHSP_eq_bind, treeCutsP_node, Multiset.cons_bind, Multiset.bind_map]
  have h1 : hL τ (({Tree.node a cs} : FP (α ⊕ β)), (0 : FP (α ⊕ β))) =
      (({Tree.node a cs} : FP (α ⊕ β)), (0 : FP (α ⊕ β)), (0 : FP (α ⊕ β))) ::ₘ
        (Cl τ cs).map (fun p => (p.1, ({Tree.node a p.2} : FP (α ⊕ β)), (0 : FP (α ⊕ β)))) := by
    show (forestCutsP τ {Tree.node a cs}).map
...
theorem dcRHSP_node (τ : Tree (α ⊕ β) → β) (a : α ⊕ β) (cs : List (Tree (α ⊕ β))) :
    dcRHSP τ (Tree.node a cs) =
      {(({Tree.node a cs} : FP (α ⊕ β)), (0 : FP (α ⊕ β)), (0 : FP (α ⊕ β)))}
      + (Cl τ cs).bind (fun p =>
          (treeCutsP τ (Tree.node a p.2)).map (fun B12 => (p.1, B12.1, B12.2))) := by
  rw [dcRHSP_eq_bind, treeCutsP_node, Multiset.cons_bind, Multiset.bind_map]
  have h1 : hR τ (({Tree.node a cs} : FP (α ⊕ β)), (0 : FP (α ⊕ β))) =
      {(({Tree.node a cs} : FP (α ⊕ β)), (0 : FP (α ⊕ β)), (0 : FP (α ⊕ β)))} := by
    show (forestCutsP τ (0 : FP (α ⊕ β))).map
        (fun B12 => (({Tree.node a cs} : FP (α ⊕ β)), B12.1, B12.2)) = _
    rw [forestCutsP_zero, Multiset.map_singleton]
  rw [h1]
  congr 1
  apply Multiset.bind_congr; intro p _
  show (forestCutsP τ {Tree.node a p.2}).map (fun B12 => (p.1, B12.1, B12.2)) = _
...
def clconv (S T : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β))) :=
  (S ×ˢ T).map (fun pr => (pr.1.1 + pr.2.1, pr.1.2 ++ pr.2.2))

theorem clconv_bind (S T : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
...
theorem clconv_unit_left (S : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    clconv {((0 : FP (α ⊕ β)), ([] : List (Tree (α ⊕ β))))} S = S := by
...
theorem clconv_unit_right (S : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    clconv S {((0 : FP (α ⊕ β)), ([] : List (Tree (α ⊕ β))))} = S := by
  rw [clconv_bind]
  rw [show (fun s : FP (α ⊕ β) × List (Tree (α ⊕ β)) =>
        ({((0 : FP (α ⊕ β)), ([] : List (Tree (α ⊕ β))))} : Multiset _).map
...
theorem clconv_assoc (S T U : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
...
theorem Cl_nil (τ : Tree (α ⊕ β) → β) :
    Cl τ ([] : List (Tree (α ⊕ β))) =
      {((0 : FP (α ⊕ β)), ([] : List (Tree (α ⊕ β))))} := cutListSummandsG_nil _

theorem Cl_cons (τ : Tree (α ⊕ β) → β) (t : Tree (α ⊕ β)) (ts : List (Tree (α ⊕ β))) :
...
theorem Cl_append (τ : Tree (α ⊕ β) → β) (l₁ l₂ : List (Tree (α ⊕ β))) :
...
def proj3L (q : FP (α ⊕ β) × FP (α ⊕ β) × List (Tree (α ⊕ β))) :
    Multiset (Nonplanar (α ⊕ β)) × Multiset (Nonplanar (α ⊕ β)) ×
      Multiset (Nonplanar (α ⊕ β)) :=
  (q.1.map Nonplanar.mk, q.2.1.map Nonplanar.mk,
    Multiset.ofList (q.2.2.map Nonplanar.mk))

theorem proj3L_add (a a' b b' : FP (α ⊕ β)) (l l' : List (Tree (α ⊕ β))) :
...
def dcl (τ : Tree (α ⊕ β) → β) (S : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    Multiset (FP (α ⊕ β) × FP (α ⊕ β) × List (Tree (α ⊕ β))) :=
...
def dcr (τ : Tree (α ⊕ β) → β) (S : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    Multiset (FP (α ⊕ β) × FP (α ⊕ β) × List (Tree (α ⊕ β))) :=
  S.bind (fun p => (Cl τ p.2).map (fun q => (p.1, q.1, q.2)))

theorem dclN_eq (τ : Tree (α ⊕ β) → β)
    (S : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    (dcl τ S).map proj3L =
...
private theorem hΦl (τ : Tree (α ⊕ β) → β) (s t : FP (α ⊕ β) × List (Tree (α ⊕ β))) :
...
private theorem hΦr (τ : Tree (α ⊕ β) → β) (s t : FP (α ⊕ β) × List (Tree (α ⊕ β))) :
...
theorem dclN_clconv (τ : Tree (α ⊕ β) → β)
    (S T : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
...
theorem dcrN_clconv (τ : Tree (α ⊕ β) → β)
    (S T : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
...
/-- Tree trace coherence: `τ` of a cut trunk equals `τ` of the tree. The
    descent of `TraceCoherent` (Nonplanar) along `Nonplanar.mk`. -/
def TraceCoherentP (τ : Tree (α ⊕ β) → β) : Prop :=
  ∀ t : Tree (α ⊕ β), ∀ p ∈ cutSummandsCP τ t, τ p.2 = τ t

/-- Children-list cut of a singleton list is the per-child augmented action. -/
theorem Cl_singleton (τ : Tree (α ⊕ β) → β) (t : Tree (α ⊕ β)) :
    Cl τ [t] = augActionG (extractC τ) t := by
  rw [show ([t] : List (Tree (α ⊕ β))) = t :: [] from rfl, Cl_cons, Cl_nil,
      clconv_unit_right]
...
theorem proj3L_list_wrap (a' : α ⊕ β) (x y : FP (α ⊕ β)) (l : List (Tree (α ⊕ β))) :
    proj3L (x, y, [Tree.node a' l])
      = ((proj3L (x, y, l)).1, (proj3L (x, y, l)).2.1,
          ({Nonplanar.node a' (proj3L (x, y, l)).2.2} : Multiset (Nonplanar (α ⊕ β)))) := by
  unfold proj3L
  simp only [List.map_cons, List.map_nil]
  rw [show (Multiset.ofList [Nonplanar.mk (Tree.node a' l)])
        = ({Nonplanar.mk (Tree.node a' l)} : Multiset (Nonplanar (α ⊕ β))) from rfl,
...
private theorem bind_cons_splitL {γ : Type*} (S : Multiset γ)
    (x : γ → FP (α ⊕ β) × FP (α ⊕ β) × List (Tree (α ⊕ β)))
    (h : γ → Multiset (FP (α ⊕ β) × FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
...
theorem dcl_INH_wrap (τ : Tree (α ⊕ β) → β) (a' : α ⊕ β) (cs'' : List (Tree (α ⊕ β))) :
    (dcl τ ((Cl τ cs'').map (fun p => (p.1, ([Tree.node a' p.2] : List (Tree (α ⊕ β))))))).map proj3L
...
theorem dcr_nonEW_wrap (τ : Tree (α ⊕ β) → β) (a' : α ⊕ β) (cs'' : List (Tree (α ⊕ β))) :
    ((Cl τ cs'').bind (fun p => (Cl τ p.2).map
        (fun p'' => (p.1, p''.1, ([Tree.node a' p''.2] : List (Tree (α ⊕ β))))))).map proj3L
...
theorem dcl_cons (τ : Tree (α ⊕ β) → β) (F : FP (α ⊕ β)) (l : List (Tree (α ⊕ β)))
    (S : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    dcl τ ((F, l) ::ₘ S)
...
theorem dcr_cons (τ : Tree (α ⊕ β) → β) (F : FP (α ⊕ β)) (l : List (Tree (α ⊕ β)))
    (S : Multiset (FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    dcr τ ((F, l) ::ₘ S)
...
mutual
theorem coassL (τ : Tree (α ⊕ β) → β) (hτ : TraceCoherentP τ) :
    ∀ cs : List (Tree (α ⊕ β)),
...
theorem coassA (τ : Tree (α ⊕ β) → β) (hτ : TraceCoherentP τ) :
    ∀ c : Tree (α ⊕ β),
  ...
  | .node a' cs'' => by
    ...
    | inr b' =>
      ...
      rw [show augActionG (extractC τ) (Tree.node (Sum.inr b') cs'')
            = (Cl τ cs'').map (fun p => (p.1, ([Tree.node (Sum.inr b') p.2]
                : List (Tree (α ⊕ β))))) from by
          rw [augActionG_eq_none _ _ (extractC_inr τ b' cs''), cutSummandsG_node,
              Multiset.map_map]; rfl]
      have hdcrINH : dcr τ ((Cl τ cs'').map
            (fun p => (p.1, ([Tree.node (Sum.inr b') p.2] : List (Tree (α ⊕ β))))))
          = (Cl τ cs'').bind (fun p => (Cl τ p.2).map
              (fun p'' => (p.1, p''.1, [Tree.node (Sum.inr b') p''.2]))) := by
        unfold dcr; rw [Multiset.bind_map]
        apply Multiset.bind_congr; intro p _
        rw [show Cl τ [Tree.node (Sum.inr b') p.2] = (Cl τ p.2).map
              (fun p'' => (p''.1, [Tree.node (Sum.inr b') p''.2])) from by
    ...
    | inl a'' =>
      have hAa : augActionG (extractC τ) (Tree.node (Sum.inl a'') cs'')
          = (({Tree.node (Sum.inl a'') cs''} : FP (α ⊕ β)),
              [traceLeaf (τ (Tree.node (Sum.inl a'') cs''))]) ::ₘ
              (Cl τ cs'').map (fun p => (p.1, [Tree.node (Sum.inl a'') p.2])) := by
        rw [augActionG_eq_some _ _ _ (extractC_inl τ a'' cs''), cutSummandsG_node,
            Multiset.map_map]; rfl
      have hCltl : Cl τ [traceLeaf (τ (Tree.node (Sum.inl a'') cs''))]
          = {((0 : FP (α ⊕ β)), [traceLeaf (τ (Tree.node (Sum.inl a'') cs''))])} := by
        rw [Cl_singleton]
        show augActionG (extractC τ)
              (Tree.node (Sum.inr (τ (Tree.node (Sum.inl a'') cs''))) [])
            = {((0 : FP (α ⊕ β)),
                [Tree.node (Sum.inr (τ (Tree.node (Sum.inl a'') cs''))) []])}
        rw [augActionG_eq_none _ _ (extractC_inr τ _ []), cutSummandsG_node,
            cutListSummandsG_nil]; rfl
      have hdcrINH : dcr τ ((Cl τ cs'').map
            (fun p => (p.1, [Tree.node (Sum.inl a'') p.2])))
          = (Cl τ cs'').map (fun p => (p.1, ({Tree.node (Sum.inl a'') p.2} : FP (α ⊕ β)),
                [traceLeaf (τ (Tree.node (Sum.inl a'') p.2))]))
            + (Cl τ cs'').bind (fun p => (Cl τ p.2).map
                (fun p'' => (p.1, p''.1, [Tree.node (Sum.inl a'') p''.2]))) := by
        have step1 : dcr τ ((Cl τ cs'').map
              (fun p => (p.1, [Tree.node (Sum.inl a'') p.2])))
            = (Cl τ cs'').bind (fun p =>
                (p.1, ({Tree.node (Sum.inl a'') p.2} : FP (α ⊕ β)),
                  [traceLeaf (τ (Tree.node (Sum.inl a'') p.2))]) ::ₘ
                (Cl τ p.2).map (fun p'' =>
                  (p.1, p''.1, [Tree.node (Sum.inl a'') p''.2]))) := by
          unfold dcr; rw [Multiset.bind_map]
          apply Multiset.bind_congr; intro p _
          show (Cl τ [Tree.node (Sum.inl a'') p.2]).map
              (fun q => (p.1, q.1, q.2)) = _
          rw [show Cl τ [Tree.node (Sum.inl a'') p.2]
                = (({Tree.node (Sum.inl a'') p.2} : FP (α ⊕ β)),
                    [traceLeaf (τ (Tree.node (Sum.inl a'') p.2))]) ::ₘ
                  (Cl τ p.2).map (fun p'' =>
                    (p''.1, [Tree.node (Sum.inl a'') p''.2])) from by
      ...
      have hcoh : ((Cl τ cs'').map (fun p : FP (α ⊕ β) × List (Tree (α ⊕ β)) => (p.1,
              ({Tree.node (Sum.inl a'') p.2} : FP (α ⊕ β)),
              [traceLeaf (τ (Tree.node (Sum.inl a'') p.2))]))).map proj3L
          = ((Cl τ cs'').map (fun p : FP (α ⊕ β) × List (Tree (α ⊕ β)) => (p.1,
              ({Tree.node (Sum.inl a'') p.2} : FP (α ⊕ β)),
              [traceLeaf (τ (Tree.node (Sum.inl a'') cs''))]))).map proj3L := by
        rw [Multiset.map_map, Multiset.map_map]
        apply Multiset.map_congr rfl; intro p hp
        simp only [Function.comp_apply]
        rw [show τ (Tree.node (Sum.inl a'') p.2) = τ (Tree.node (Sum.inl a'') cs'') from
          hτ (Tree.node (Sum.inl a'') cs'') (p.1, Tree.node (Sum.inl a'') p.2)
      ...
      have hΦdcl : ((forestCutsP τ {Tree.node (Sum.inl a'') cs''}).map
            (fun c12 => (c12.1, c12.2,
              [traceLeaf (τ (Tree.node (Sum.inl a'') cs''))]))).map proj3L
          = proj3L (({Tree.node (Sum.inl a'') cs''} : FP (α ⊕ β)), (0 : FP (α ⊕ β)),
              [traceLeaf (τ (Tree.node (Sum.inl a'') cs''))]) ::ₘ
            ((Cl τ cs'').map (fun p : FP (α ⊕ β) × List (Tree (α ⊕ β)) => (p.1,
              ({Tree.node (Sum.inl a'') p.2} : FP (α ⊕ β)),
              [traceLeaf (τ (Tree.node (Sum.inl a'') cs''))]))).map proj3L := by
...
private theorem proj3_node_wrap (a : α ⊕ β)
    (q : FP (α ⊕ β) × FP (α ⊕ β) × List (Tree (α ⊕ β))) :
    proj3 (q.1, q.2.1, ({Tree.node a q.2.2} : FP (α ⊕ β)))
...
private theorem map_node_wrap_proj3 (a : α ⊕ β)
    (M : Multiset (FP (α ⊕ β) × FP (α ⊕ β) × List (Tree (α ⊕ β)))) :
    (M.map (fun q => (q.1, q.2.1, ({Tree.node a q.2.2} : FP (α ⊕ β))))).map proj3
...
theorem coassT (τ : Tree (α ⊕ β) → β) (hτ : TraceCoherentP τ) (t : Tree (α ⊕ β)) :
  ...
  have hcore : ((Cl τ cs).bind (fun p => (forestCutsP τ p.1).map
        (fun c12 => (c12.1, c12.2, ({Tree.node a p.2} : FP (α ⊕ β)))))).map proj3
      = ((Cl τ cs).bind (fun p => (Cl τ p.2).map
        (fun q => (p.1, q.1, ({Tree.node a q.2} : FP (α ⊕ β)))))).map proj3 := by
    rw [show ((Cl τ cs).bind (fun p => (forestCutsP τ p.1).map
            (fun c12 => (c12.1, c12.2, ({Tree.node a p.2} : FP (α ⊕ β))))))
          = ((dcl τ (Cl τ cs)).map
              (fun q => (q.1, q.2.1, ({Tree.node a q.2.2} : FP (α ⊕ β))))) from by
        unfold dcl; rw [Multiset.map_bind]
        apply Multiset.bind_congr; intro p _; rw [Multiset.map_map]; rfl,
      show ((Cl τ cs).bind (fun p => (Cl τ p.2).map
            (fun q => (p.1, q.1, ({Tree.node a q.2} : FP (α ⊕ β))))))
          = ((dcr τ (Cl τ cs)).map
              (fun q => (q.1, q.2.1, ({Tree.node a q.2.2} : FP (α ⊕ β))))) from by
        unfold dcr; rw [Multiset.map_bind]
        apply Multiset.bind_congr; intro p _; rw [Multiset.map_map]; rfl]
  ...
  rw [show (fun p : FP (α ⊕ β) × List (Tree (α ⊕ β)) =>
        (treeCutsP τ (Tree.node a p.2)).map (fun B12 => (p.1, B12.1, B12.2)))
        = (fun p => (p.1, ({Tree.node a p.2} : FP (α ⊕ β)), (0 : FP (α ⊕ β))) ::ₘ
            (Cl τ p.2).map (fun q => (p.1, q.1, ({Tree.node a q.2} : FP (α ⊕ β))))) from by
      funext p
