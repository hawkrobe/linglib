...
/-- The Δ^c extraction policy: for `Sum.inl`-rooted (non-trace)
    ...
    to extract. -/
def extractC (τ : Planar (α ⊕ β) → β) :
    Planar (α ⊕ β) → Option (List (Planar (α ⊕ β)))
  | t@(.node (Sum.inl _) _) => some [traceLeaf (τ t)]
  | .node (Sum.inr _) _ => none

@[simp] theorem extractC_inl (τ : Planar (α ⊕ β) → β)
    (a : α) (cs : List (Planar (α ⊕ β))) :
    extractC τ (Planar.node (Sum.inl a) cs) =
      some [traceLeaf (τ (Planar.node (Sum.inl a) cs))] := rfl

@[simp] theorem extractC_inr (τ : Planar (α ⊕ β) → β)
    (b : β) (cs : List (Planar (α ⊕ β))) :
    extractC τ (Planar.node (Sum.inr b) cs) = none := rfl
...
def cutSummandsCP (τ : Planar (α ⊕ β) → β) :
    Planar (α ⊕ β) → Multiset (Forest (Planar (α ⊕ β)) × Planar (α ⊕ β)) :=
  cutSummandsG (extractC τ)

theorem cutSummandsCP_def (τ : Planar (α ⊕ β) → β) (T : Planar (α ⊕ β)) :
...
example (τ : Planar (Unit ⊕ Unit) → Unit) :
  ...
  rw [cutSummandsCP_node, cutListSummandsG_nil]
  rfl
    ...
    (({Planar.leaf (Sum.inl ())} : Forest (Planar (Unit ⊕ Unit))),
      [traceLeaf (τ (Planar.leaf (Sum.inl ())))]) ∈
        augActionG (extractC τ)
          (Planar.leaf (Sum.inl ()) : Planar (Unit ⊕ Unit)) := by
  show _ ∈ augActionG (extractC τ) (Planar.node (Sum.inl ()) [])
  rw [augActionG_eq_some _ _ _ (extractC_inl τ () [])]
...
example (b : Unit) (τ : Planar (Unit ⊕ Unit) → Unit) :
    augActionG (extractC τ) (traceLeaf b : Planar (Unit ⊕ Unit))
      = (cutSummandsG (extractC τ) (Planar.node (Sum.inr b) [])).map
          (fun p => (p.1, [p.2])) := by
  show augActionG _ (Planar.node (Sum.inr b) []) = _
  rw [augActionG_eq_none _ _ (extractC_inr τ b [])]
