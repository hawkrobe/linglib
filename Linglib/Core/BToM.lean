...
structure BToMModel (F : Type*) [CommSemiring F]
  ...
  worldPrior : World → F
  /-- Prior over desires: P(d). -/
  desirePrior : Desire → F
  ...
  /-- How shared state evolves after an action.
  ...
  sharedUpdate : Shared → Action → Shared := λ s _ => s
...
given an observed action:
    ...
    joint(a, p, b, d, s, m, w) =
      ...
      worldPrior(w) · desirePrior(d) · sharedPrior(s) · mediumPrior(m)
...
theorem BToMModel.worldMarginal_eq (model : BToMModel F A P B D S M W)
    ...
    model.worldPrior w * ∑ p : P, ∑ b : B, ∑ d : D, ∑ s : S, ∑ m : M,
      ...
      model.desirePrior d * model.sharedPrior s * model.mediumPrior m := by
  ...
  trans (∑ p : P, ∑ b : B, ∑ d : D, ∑ s : S, ∑ m : M,
    ...
    model.perceptModel w p * model.desirePrior d * model.sharedPrior s *
    model.mediumPrior m))
