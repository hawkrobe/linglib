...
theorem traceLeafCount_node_of_ne_nil (v : α ⊕ β) (cs : List (Tree (α ⊕ β)))
...
@[simp] theorem traceLeafCount_node_inl (a : α) (cs : List (Tree (α ⊕ β))) :
...
private theorem traceLeafCount_permStep {t s : Tree (α ⊕ β)}
...
theorem traceLeafCount_permEquiv {t s : Tree (α ⊕ β)}
...
def traceDepthSumList : List (Tree (α ⊕ β)) → ℕ
  | []      => 0
  | t :: ts => traceDepthSum t + traceDepthSumList ts
end

theorem traceDepthSum_node (v : α ⊕ β) (cs : List (Tree (α ⊕ β))) :
    traceDepthSum (.node v cs) = traceDepthSumList cs + (cs.map traceLeafCount).sum := rfl

@[simp] theorem traceDepthSumList_nil :
    traceDepthSumList ([] : List (Tree (α ⊕ β))) = 0 := rfl

@[simp] theorem traceDepthSumList_cons (t : Tree (α ⊕ β)) (ts : List (Tree (α ⊕ β))) :
    traceDepthSumList (t :: ts) = traceDepthSum t + traceDepthSumList ts := rfl

@[simp] theorem traceDepthSum_leaf_inl (a : α) :
    traceDepthSum (.node (Sum.inl a) [] : Tree (α ⊕ β)) = 0 := rfl

@[simp] theorem traceDepthSum_leaf_inr (b : β) :
    traceDepthSum (.node (Sum.inr b) [] : Tree (α ⊕ β)) = 0 := rfl

theorem traceDepthSumList_eq_sum_map (cs : List (Tree (α ⊕ β))) :
...
theorem traceDepthSum_node_eq (v : α ⊕ β) (cs : List (Tree (α ⊕ β))) :
...
private theorem traceDepthSum_permStep {t s : Tree (α ⊕ β)}
...
theorem traceDepthSum_permEquiv {t s : Tree (α ⊕ β)}
...
theorem traceLeafCount_le_node (v : α ⊕ β) (cs : List (Tree (α ⊕ β))) :
...
theorem traceLeafCount_le_traceDepthSum_of_inl (a : α) (cs : List (Tree (α ⊕ β))) :
...
/-- The number of `Sum.inr`-labeled (trace-marker) leaves of a nonplanar
    tree, lifted from `Tree.traceLeafCount`. -/
def traceLeafCount : Nonplanar (α ⊕ β) → ℕ :=
  Nonplanar.lift Tree.traceLeafCount
    (fun _ _ h => Tree.traceLeafCount_permEquiv h)

@[simp] theorem traceLeafCount_mk (t : Tree (α ⊕ β)) :
...
/-- The **Minimal Search depth** `d_v` of MCB §1.5: the sum of root-distances
    of the trace-marker leaves, lifted from `Tree.traceDepthSum`. For a Δ^c
    cut, `traceDepthSum` of the quotient is the total extraction depth. -/
def traceDepthSum : Nonplanar (α ⊕ β) → ℕ :=
  Nonplanar.lift Tree.traceDepthSum
    (fun _ _ h => Tree.traceDepthSum_permEquiv h)

@[simp] theorem traceDepthSum_mk (t : Tree (α ⊕ β)) :
...
@[simp] theorem traceDepthSum_node_inl (a : α) (F : Multiset (Nonplanar (α ⊕ β))) :
  ...
  rw [traceDepthSum_mk, Tree.traceDepthSum_node_eq, List.map_map]
  ...
  show Tree.traceDepthSum (Quotient.out t) + Tree.traceLeafCount (Quotient.out t)
...
@[simp] theorem traceLeafCount_node_inl (a : α) (F : Multiset (Nonplanar (α ⊕ β))) :
  ...
  rw [traceLeafCount_mk, Tree.traceLeafCount_node_inl, List.map_map]
  simp only [Multiset.quot_mk_to_coe, Multiset.map_coe, Multiset.sum_coe]
  refine congrArg List.sum (List.map_congr_left fun t _ => ?_)
