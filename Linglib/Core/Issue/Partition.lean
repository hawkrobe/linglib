...
/-!
# Issue — partition predicate, Setoid bridge, QUD bridge
@cite{groenendijk-stokhof-1984} @cite{roberts-2012}
@cite{ciardelli-groenendijk-roelofsen-2018}

Bridge from `Issue W` (downward-closed nonempty families of information
...
- `isPartition_polar`, `isPartition_ofList` — the basic Hamblin
  ...
  / cover hypotheses.
- `toSetoid_fromSetoid` — `Setoid → Issue → Setoid` is the identity
...
def IsPartition (P : Issue W) : Prop :=
...
/-- Partition issues are **non-informative**: every world is covered by
    ...
    `info P = Set.univ`. -/
@[simp] theorem info_eq_univ {P : Issue W} (h : P.IsPartition) :
    info P = Set.univ := by
...
theorem pairwiseDisjoint {P : Issue W} (h : P.IsPartition) :
    (alt P).PairwiseDisjoint id :=
  Setoid.IsPartition.pairwiseDisjoint h

/-- Alternatives of a partition issue are nonempty. Mathlib's
    `Setoid.nonempty_of_mem_partition` specialized to `Issue`. -/
theorem nonempty_of_mem_alt {P : Issue W} (h : P.IsPartition)
...
theorem sUnion_alt_eq_univ {P : Issue W} (h : P.IsPartition) :
    ⋃₀ alt P = Set.univ :=
...
def toSetoid {P : Issue W} (h : P.IsPartition) : Setoid W :=
...
/-- The equivalence classes of `toSetoid h` are exactly the
    ...
    `Setoid.mkClasses`. -/
@[simp] theorem classes_toSetoid {P : Issue W} (h : P.IsPartition) :
...
theorem toSetoid_rel_iff {P : Issue W} (h : P.IsPartition) (w v : W) :
...
/-- Bridge to legacy Bool-based `QUD W`: two worlds get the same
    ...
    classically. -/
noncomputable def toQUD {P : Issue W} (h : P.IsPartition) : QUD W := by
  classical
  refine
    { sameAnswer := fun w v => decide ((toSetoid h) w v)
      refl := ?_
      symm := ?_
      trans := ?_ }
  · intro w
    exact decide_eq_true_eq.mpr ((toSetoid h).iseqv.refl w)
  · intro w v
    rw [decide_eq_decide]
    exact ⟨(toSetoid h).iseqv.symm, (toSetoid h).iseqv.symm⟩
  · intro w v u h1 h2
    rw [decide_eq_true_eq] at *
    exact (toSetoid h).iseqv.trans h1 h2

/-- `toQUD`'s `sameAnswer` is true iff the two worlds share a cell. -/
theorem toQUD_sameAnswer_iff {P : Issue W} (h : P.IsPartition) (w v : W) :
    (toQUD h).sameAnswer w v = true ↔ ∃ p ∈ alt P, w ∈ p ∧ v ∈ p := by
  classical
  show decide ((toSetoid h) w v) = true ↔ _
  rw [decide_eq_true_eq, toSetoid_rel_iff]

/-- The cell of a world under `toQUD h` is in `alt P`. -/
theorem cell_toQUD_mem_alt {P : Issue W} (h : P.IsPartition) (w : W) :
...
theorem mem_cell_toQUD {P : Issue W} (h : P.IsPartition) (w : W) :
    w ∈ (toQUD h).cell w :=
  QUD.cell_self _ w

/-- The `toQUD` cell of any world in an alternative `p` is exactly `p`. -/
theorem cell_toQUD_eq {P : Issue W} (h : P.IsPartition) {p : Set W}
...
The `Setoid → Issue` embedding (`fromSetoid`, in
`Core/Mood/PartitionAsInquiry.lean`) and `Issue → Setoid` projection
...
/-- **Round-trip**: the setoid derived from a partition issue built
    from a setoid recovers the original setoid. The `Setoid → Issue →
...
/-- `Issue.polar p` is a partition issue when `p` is non-trivial: the
...
/-- `Issue.ofList L` is a partition issue when `L` is a nonempty list of
