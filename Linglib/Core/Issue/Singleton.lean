...
/-!
# Issue — singleton-alternative predicate
@cite{bhatt-dayal-2020} @cite{roelofsen-farkas-2015}

A foundational predicate over `Issue W`: an issue is *singleton* iff its
...
def IsSingleton (P : Issue W) : Prop :=
  ∃ p, alt P = {p}
...
@[simp] theorem isSingleton_top : IsSingleton (⊤ : Issue W) :=
  ⟨Set.univ, alt_top⟩

@[simp] theorem isSingleton_bot : IsSingleton (⊥ : Issue W) :=
  ⟨∅, alt_bot⟩
...
theorem isSingleton_iff_subsingleton_and_nonempty (P : Issue W) :
...
/-- A declarative content is singleton with witness `info P`: combines
    ...
    `IsSingleton`. -/
theorem IsSingleton.of_isDeclarative {P : Issue W}
...
/-- **Two-distinct-alternatives obstruction**: if `P` has two distinct
    ...
    case below. -/
theorem not_isSingleton_of_two_alternatives (P : Issue W)
...
/-- The **subtype of singleton issues** — issues whose alternative set
    ...
    use a subtype. -/
def SingletonIssue (W : Type u) : Type u :=
  {Q : Issue W // IsSingleton Q}

namespace SingletonIssue

/-- The underlying issue of a `SingletonIssue` — the truth-conditional
