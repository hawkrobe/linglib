...
/-!
# Issue — granularity and the width relation
...
`P.widerThan Q` (over the Set/Issue lattice) holds when:
...
the Deo–Thomas Bool/List definition over `Core.Issue`'s native maximal-
...
def widerThan (P Q : Issue W) : Prop :=
  P.info = Q.info ∧
  (∀ p₂ ∈ alt Q, ∀ p₁ ∈ alt P, ¬ (p₂ ⊂ p₁)) ∧
  (∃ p₁ ∈ alt P, ∃ p₂ ∈ alt Q, p₁ ⊂ p₂)
