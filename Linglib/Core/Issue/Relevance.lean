...
/-!
# Roberts (2012) QUD relevance — Prop predicates on `Core.Issue`
...
list-of-alternatives `Issue` type.

The predicates operate directly on `Core.Issue`, ranging over `alt P`
(the maximal-resolving propositions). For computability, an `Issue`
...
`ofList` smart constructor (`Core/Issue/Hamblin.lean`).
...
/-- `σ` partially answers `P`: settles at least one alternative either
    ...
    `Core.Issue.isPartialAnswer` from `Core/Issue/Answerhood.lean`. -/
def partiallyAnswers (P : Issue W) (σ : Set W) : Prop :=
  isPartialAnswer P σ
...
/-- Question entailment: every alternative of `P` entails some alternative
    ...
    `Q`'s answer. -/
def questionEntails (P Q : Issue W) : Prop :=
  ∀ p ∈ alt P, ∃ q ∈ alt Q, p ⊆ q

/-- `q` is a **subquestion** of `parent`: answering `parent` settles `q`. -/
def isSubquestion (q parent : Issue W) : Prop :=
...
/-- A discourse move `den` is **relevant** to the QUD if some alternative
    ...
    assumption iii. -/
def moveRelevant (den qud : Issue W) (subquestions : List (Issue W)) : Prop :=
  ∃ a ∈ alt den,
    partiallyAnswers qud a ∨ ∃ q ∈ subquestions, partiallyAnswers q a

/-! ### Reflexivity / transitivity -/

theorem questionEntails_refl (P : Issue W) : questionEntails P P :=
  fun p hp => ⟨p, hp, Set.Subset.refl p⟩

theorem questionEntails_trans {P Q R : Issue W}
...
theorem questionEntails_of_le {P Q : Issue W} (h : P ≤ Q)
...
theorem questionEntails_of_le' [Fintype W] [DecidableEq W]
    {P Q : Issue W} (h : P ≤ Q) : questionEntails P Q := by
  haveI : Finite (Set W) := Finite.of_fintype _
  exact questionEntails_of_le h (Set.toFinite Q.props)

theorem isSubquestion_refl (P : Issue W) : isSubquestion P P :=
  questionEntails_refl P

theorem isSubquestion_trans {q r s : Issue W}
...
theorem partiallyAnswers_imp_moveRelevant
    {den qud : Issue W} {a : Set W} (ha : a ∈ alt den)
...
Mathlib provides `Set.Subset` decidability only via `Finset`. For `Issue`
...
/-- A finite-presentation witness for an `Issue`'s alternatives. The
    `altList` is a `List (Set W)` whose elements correspond bijectively
    (modulo set-equality) to `alt P`. -/
class HasAltList (P : Issue W) where
...
instance partiallyAnswers_decidable
    (P : Issue W) [HP : HasAltList P] (σ : Set W)
...
instance questionEntails_decidable
    (P Q : Issue W) [HP : HasAltList P] [HQ : HasAltList Q]
...
instance isSubquestion_decidable
    (q parent : Issue W) [HasAltList q] [HasAltList parent]
...
instance moveRelevant_decidable
    (den qud : Issue W) (subquestions : List (Issue W))
    [HD : HasAltList den]
    [∀ a : Set W, Decidable (partiallyAnswers qud a)]
    [∀ q : Issue W, ∀ a : Set W, Decidable (partiallyAnswers q a)] :
...
@[reducible]
def HasAltList.polar (p : Set W)
    (hne : p ≠ ∅) (hnu : p ≠ Set.univ) :
    HasAltList (Issue.polar p) where
  altList := [p, pᶜ]
  mem_altList q := by
...
/-- `HasAltList` witness for a Hamblin question built from an explicit
    ...
    The Hamblin construction `Issue.ofList L` has `alt = {p | p ∈ L}`
...
def HasAltList.ofList (L : List (Set W)) (hL : L ≠ [])
    ...
    (hne : ∀ p ∈ L, p ≠ ∅) :
    HasAltList (Issue.ofList L) where
  altList := L
  mem_altList q := by
...
theorem partiallyAnswers_polar_iff {p σ : Set W}
    (hne : p ≠ ∅) (hnu : p ≠ Set.univ) :
    partiallyAnswers (Issue.polar p) σ ↔ σ ⊆ p ∨ σ ⊆ pᶜ := by
...
theorem questionEntails_polar_polar_iff {p q : Set W}
    ...
    questionEntails (Issue.polar p) (Issue.polar q) ↔
...
theorem moveRelevant_polar_iff {p : Set W} {qud : Issue W}
    {subquestions : List (Issue W)}
    (hne : p ≠ ∅) (hnu : p ≠ Set.univ) :
    moveRelevant (Issue.polar p) qud subquestions ↔
