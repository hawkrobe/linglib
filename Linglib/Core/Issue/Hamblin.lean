...
/-- The **wh-question** content for "Which x ∈ D satisfies P x?". One
    ...
    declarative (P e)`. -/
def which {E : Type v} (D : Set E) (P : E → Set W) : Issue W :=
  ⨆ e ∈ D, declarative (P e)
...
into a `Core.Issue W`, mediating between abstract Set-based issues and
...
def ofList (L : List (Set W)) : Issue W :=
