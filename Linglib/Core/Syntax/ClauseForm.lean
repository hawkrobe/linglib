...
`Core.Mood.ClauseType` (`Linglib.Core.Mood.ClauseType`), which pairs
illocutionary force with verbal mood. The two complement each other:
a polar question has `ClauseForm = matrixQuestion` and
`Core.Mood.ClauseType = ⟨interrogative, indicative⟩`.
-/
