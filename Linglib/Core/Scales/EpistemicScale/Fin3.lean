...
/-!
# Theorem 8a for Fin 3

Every FA system on 3 elements is representable by a finitely additive probability
measure. The proof constructs an explicit measure witness by case-splitting on
null patterns and pairwise orderings of singletons.

## Strategy

1. **Null reduction**: If any singleton is null (ge ∅ {i}), reduce to Fin 2 via
   `null_elem_reduce` + `theorem8a_fin2`.
2. **0-null case** (all singletons positive): Case-split on pairwise orderings
   (ge {i} {j}). For each of the 13 distinct ordering patterns, construct an
   explicit rational measure (a, b, 1-a-b) and verify all 64 disjoint-pair
   comparisons via `fin3_dispatch`.
3. **Permutation routing**: WLOG reduce symmetric null patterns to canonical
   ones via `perm_repr`.
-/
