...
theorem fin4_case_31 (sys : EpistemicSystemFA (Fin 4))
  ...
  have hd34 : sys.ge ({0,2} : Set (Fin 4)) ({1,3} : Set _) := by
    ...
    have h2 : sys.ge ({0,1} : Set _) ({1,3} : Set _) := by
      ...
      exact h03
    exact sys.trans _ _ _ h1 h2
  have hd35 : sys.ge ({1,2} : Set (Fin 4)) ({0,3} : Set _) := by
    ...
    have h2 : sys.ge ({0,1} : Set _) ({0,3} : Set _) := by
      ...
      exact h13
    exact sys.trans _ _ _ h1 h2
  have hd36 : ¬sys.ge ({2,3} : Set (Fin 4)) ({0,1} : Set _) := fun h => by
    ...
    exact hng30 h1
  have hd37 : ¬sys.ge ({1,3} : Set (Fin 4)) ({0,2} : Set _) := fun h => by
    ...
    exact hng30 h1
  have hd38 : ¬sys.ge ({0,3} : Set (Fin 4)) ({1,2} : Set _) := fun h => by
    have h_bc : sys.ge ({1,2} : Set (Fin 4)) ({0,1} : Set _) := by
      rw [sys.additive ({1,2} : Set (Fin 4)) {0,1},
          show ({1,2} : Set (Fin 4)) \ {0,1} = {(2 : Fin 4)} from by ext x; fin_cases x <;> simp_all,
          show ({0,1} : Set (Fin 4)) \ {1,2} = {(0 : Fin 4)} from by ext x; fin_cases x <;> simp_all]
      exact h20
    ...
    rw [sys.additive ({0,3} : Set (Fin 4)) {0,1},
        show ({0,3} : Set (Fin 4)) \ {0,1} = {(3 : Fin 4)} from by ext x; fin_cases x <;> simp_all,
        show ({0,1} : Set (Fin 4)) \ {0,3} = {(1 : Fin 4)} from by ext x; fin_cases x <;> simp_all] at h1
    exact hng31 h1
  exact fin4_witness sys (11/42) (11/42) (11/42) (by linarith) (by linarith) (by linarith) (by linarith)
    ...
    (fin4_dispatch sys (11/42) (11/42) (11/42) (by linarith) (by linarith) (by linarith) (by linarith)
      ...
      ⟨fun h => absurd h hd36, fun h => by linarith⟩
