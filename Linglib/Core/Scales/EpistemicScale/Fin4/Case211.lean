...
theorem fin4_case_211 (sys : EpistemicSystemFA (Fin 4))
  ...
  have hd30 : sys.ge ({0,2} : Set (Fin 4)) ({1,3} : Set _) := by
    ...
    have h2 : sys.ge ({0,3} : Set _) ({1,3} : Set _) := by
      ...
      exact h01
    exact sys.trans _ _ _ h1 h2
  have hd31 : sys.ge ({1,2} : Set (Fin 4)) ({0,3} : Set _) := by
    ...
    have h2 : sys.ge ({0,2} : Set _) ({0,3} : Set _) := by
      ...
      exact h23
    exact sys.trans _ _ _ h1 h2
  have hd32 : ¬sys.ge ({2,3} : Set (Fin 4)) ({0,1} : Set _) := fun h => by
    ...
    exact hng30 h1
  have hd33 : ¬sys.ge ({1,3} : Set (Fin 4)) ({0,2} : Set _) := fun h => by
    have h_bc : sys.ge ({0,2} : Set (Fin 4)) ({1,2} : Set _) := by
    ...
    rw [sys.additive ({1,3} : Set (Fin 4)) {1,2},
        ...
        show ({1,2} : Set (Fin 4)) \ {1,3} = {(2 : Fin 4)} from by ext x; fin_cases x <;> simp_all] at h1
    exact hng32 h1
  have hd34 : ¬sys.ge ({0,3} : Set (Fin 4)) ({1,2} : Set _) := fun h => by
