...
private theorem perm_null_iff {n : ℕ} (σ : Fin n ≃ Fin n)
...
private theorem perm_null_convert {n : ℕ} (σ : Fin n ≃ Fin n)
...
/-- Permutation transport for representability: if `transportFA σ sys` is representable,
    then so is `sys`. -/
private theorem perm_repr {n : ℕ} (σ : Fin n ≃ Fin n) (sys : EpistemicSystemFA (Fin n))
