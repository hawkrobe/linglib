...
theorem klDiv_eq_ofReal_integral_llr (hμν : μ ≪ ν)
...
theorem klDiv_of_llr_ae_const (hμν : μ ≪ ν) {r : ℝ}
    (h : llr μ ν =ᵐ[μ] fun _ => r) :
...
theorem klDiv_of_rnDeriv_ae_const (hμν : μ ≪ ν) {c : ℝ≥0∞}
