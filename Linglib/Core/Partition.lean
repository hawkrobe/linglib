...
/-- Convert a QUD to an Issue by computing partition cells over a finite domain.

    Inverse of `Discourse.Issue.toQUD`: the cells of the QUD become the
    alternatives of the Issue. -/
def toIssue {W : Type*} (q : QUD W) (worlds : List W) : Discourse.Issue W :=
  ⟨q.toCells worlds⟩

private lemma filter_length_one_eq {α : Type*} (l : List α) (p : α → Bool)
    (h : (l.filter p).length = 1)
    {a b : α} (ha : a ∈ l) (hpa : p a = true) (hb : b ∈ l) (hpb : p b = true) :
    a = b := by
  have ha' : a ∈ l.filter p := List.mem_filter.mpr ⟨ha, hpa⟩
  have hb' : b ∈ l.filter p := List.mem_filter.mpr ⟨hb, hpb⟩
  obtain ⟨c, t, heq⟩ := List.exists_of_length_succ _ h
  rw [heq] at h ha' hb'
  simp only [List.length_cons] at h
  have ht : t = [] := by match t, h with | [], _ => rfl
  subst ht
  rw [List.mem_singleton] at ha' hb'
  exact ha'.trans hb'.symm

private lemma partition_filter_one {W : Type*} (q : Discourse.Issue W) (worlds : List W)
    (hp : q.isPartition worlds = true) (w : W) (hw : w ∈ worlds) :
    (q.alternatives.filter (· w)).length = 1 := by
  simp only [Discourse.Issue.isPartition] at hp
  exact beq_iff_eq.mp (List.all_eq_true.mp hp w hw)

private lemma partition_has_true {W : Type*} (q : Discourse.Issue W) (worlds : List W)
    (hp : q.isPartition worlds = true) (w : W) (hw : w ∈ worlds) :
    ∃ alt ∈ q.alternatives, alt w = true := by
  have hlen := partition_filter_one q worlds hp w hw
  obtain ⟨c, t, heq⟩ := List.exists_of_length_succ _ hlen
  have hc : c ∈ q.alternatives.filter (· w) := by
    rw [heq]; exact List.mem_cons_self ..
  rw [List.mem_filter] at hc
  exact ⟨c, hc.1, hc.2⟩

private lemma partition_unique {W : Type*} (q : Discourse.Issue W) (worlds : List W)
    (hp : q.isPartition worlds = true) (w : W) (hw : w ∈ worlds)
    {a b : Discourse.InfoState W} (ha : a ∈ q.alternatives) (hpa : a w = true)
    (hb : b ∈ q.alternatives) (hpb : b w = true) : a = b :=
  filter_length_one_eq q.alternatives (· w) (partition_filter_one q worlds hp w hw) ha hpa hb hpb

private lemma partition_false_of_ne {W : Type*} (q : Discourse.Issue W) (worlds : List W)
    (hp : q.isPartition worlds = true) (w : W) (hw : w ∈ worlds)
    {alt₁ a : Discourse.InfoState W}
    (halt₁ : alt₁ ∈ q.alternatives) (halt₁w : alt₁ w = true)
    (ha : a ∈ q.alternatives) (hne : a ≠ alt₁) : a w = false := by
  cases haw : a w
  · rfl
  · exact absurd (partition_unique q worlds hp w hw ha haw halt₁ halt₁w) hne

private lemma refinesOn_imp {M : Type*} (q q' : QUD M) (worlds : List M)
    (h : Discourse.refinesOn q q' worlds = true)
    (w : M) (hw : w ∈ worlds) (v : M) (hv : v ∈ worlds)
    (hsame : q.sameAnswer w v = true) : q'.sameAnswer w v = true := by
  have hh := List.all_eq_true.mp (show worlds.all _ = true from h) w hw
  have hhv := List.all_eq_true.mp hh v hv
  simp only [Bool.or_eq_true, Bool.not_eq_true'] at hhv
  rcases hhv with hhv | hhv
  · exact absurd hsame (by simp [hhv])
  · exact hhv

private lemma backward {W : Type*} (q₁ q₂ : Discourse.Issue W) (worlds : List W)
    (hp₁ : q₁.isPartition worlds = true)
    (hp₂ : q₂.isPartition worlds = true)
    (hent : Discourse.questionEntails q₁ q₂ worlds = true) :
    Discourse.refinesOn (q₁.toQUD) (q₂.toQUD) worlds = true := by
  rw [show Discourse.refinesOn (q₁.toQUD) (q₂.toQUD) worlds =
    worlds.all (fun w => worlds.all (fun v =>
      !(q₁.toQUD).sameAnswer w v || (q₂.toQUD).sameAnswer w v)) from rfl]
  rw [List.all_eq_true]; intro w hw
  rw [List.all_eq_true]; intro v hv
  simp only [Bool.or_eq_true, Bool.not_eq_true']
  cases hsame : (q₁.toQUD).sameAnswer w v
  · left; rfl
  · right
    simp only [Discourse.Issue.toQUD, ofProject_sameAnswer, beq_iff_eq] at hsame ⊢
    apply List.map_congr_left
    intro alt₂ halt₂
    obtain ⟨alt₁, halt₁, halt₁w⟩ := partition_has_true q₁ worlds hp₁ w hw
    have halt₁v : alt₁ v = true := by
      have := List.map_eq_map_iff.mp hsame alt₁ halt₁
      rw [halt₁w] at this; exact this.symm
    obtain ⟨alt₂', halt₂', hpe⟩ : ∃ a ∈ q₂.alternatives, Discourse.propEntails alt₁ a worlds = true :=
      List.any_eq_true.mp (List.all_eq_true.mp hent alt₁ halt₁)
    have entails_at : ∀ u ∈ worlds, alt₁ u = true → alt₂' u = true := by
      intro u hu h1u
      have h := List.all_eq_true.mp hpe u hu
      simp only [Bool.or_eq_true, Bool.not_eq_true'] at h
      rcases h with h | h
      · exact absurd h1u (by simp [h])
      · exact h
    have halt₂'w := entails_at w hw halt₁w
    have halt₂'v := entails_at v hv halt₁v
    cases haw : alt₂ w
    · cases hav : alt₂ v
      · rfl
      · exfalso
        have := partition_unique q₂ worlds hp₂ v hv halt₂ hav halt₂' halt₂'v
        rw [this] at haw; simp [halt₂'w] at haw
    · rw [partition_unique q₂ worlds hp₂ w hw halt₂ haw halt₂' halt₂'w]
      exact halt₂'v.symm

private lemma forward {W : Type*} (q₁ q₂ : Discourse.Issue W) (worlds : List W)
    (hp₁ : q₁.isPartition worlds = true)
    (hp₂ : q₂.isPartition worlds = true)
    (hne : worlds ≠ [])
    (href : Discourse.refinesOn (q₁.toQUD) (q₂.toQUD) worlds = true) :
    Discourse.questionEntails q₁ q₂ worlds = true := by
  rw [Discourse.questionEntails, List.all_eq_true]
  intro alt₁ halt₁
  rw [List.any_eq_true]
  by_cases hex : ∃ w₀ ∈ worlds, alt₁ w₀ = true
  · obtain ⟨w₀, hw₀, halt₁w₀⟩ := hex
    obtain ⟨alt₂, halt₂, halt₂w₀⟩ := partition_has_true q₂ worlds hp₂ w₀ hw₀
    refine ⟨alt₂, halt₂, ?_⟩
    rw [Discourse.propEntails, List.all_eq_true]
    intro w hw
    simp only [Bool.or_eq_true, Bool.not_eq_true']
    cases halt₁w : alt₁ w
    · left; rfl
    · right
      have hprofile : q₁.alternatives.map (· w) = q₁.alternatives.map (· w₀) := by
        apply List.map_congr_left
        intro a ha
        by_cases heq : a = alt₁
        · subst heq; rw [halt₁w, halt₁w₀]
        · have hw' := partition_false_of_ne q₁ worlds hp₁ w hw halt₁ halt₁w ha heq
          have hw₀' := partition_false_of_ne q₁ worlds hp₁ w₀ hw₀ halt₁ halt₁w₀ ha heq
          rw [hw', hw₀']
      have hsame : (q₁.toQUD).sameAnswer w w₀ = true := by
        simp only [Discourse.Issue.toQUD, ofProject_sameAnswer, beq_iff_eq]
        exact hprofile
      have hq₂same := refinesOn_imp (q₁.toQUD) (q₂.toQUD) worlds href w hw w₀ hw₀ hsame
      simp only [Discourse.Issue.toQUD, ofProject_sameAnswer, beq_iff_eq] at hq₂same
      have := List.map_eq_map_iff.mp hq₂same alt₂ halt₂
      rw [halt₂w₀] at this; exact this
  · push Not at hex
    obtain ⟨w₀, hw₀⟩ := List.exists_mem_of_ne_nil worlds hne
    obtain ⟨alt₂, halt₂, _⟩ := partition_has_true q₂ worlds hp₂ w₀ hw₀
    refine ⟨alt₂, halt₂, ?_⟩
    rw [Discourse.propEntails, List.all_eq_true]
    intro w hw
    simp only [Bool.or_eq_true, Bool.not_eq_true']
...
/-- For genuine partitions, decidable refinement (`refinesOn`) and question
    entailment (`questionEntails`) coincide.

    This is the central result connecting the two question representations
    in the library: the `QUD` equivalence-relation partition and the `Issue`
    alternatives list represent the same mathematical object
    (@cite{groenendijk-stokhof-1984}).

    - **Forward** (refinement → entailment): If worlds with the same
      q₁-alternative-profile always have the same q₂-profile, then each
      q₁-cell (= alternative, since q₁ is a partition) is contained in some
      q₂-cell. Containment of cells = propositional entailment of alternatives.
    - **Backward** (entailment → refinement): If each q₁-alternative entails
      some q₂-alternative, then worlds satisfying the same q₁-alternative
      must satisfy the same q₂-alternative. Same-alternative = same-profile
      because the issues are partitions (exactly one alternative true per world).

    Requires `worlds ≠ []` for the forward direction: with empty worlds,
    `refinesOn` is vacuously true but `questionEntails` can be false when
    q₁ has alternatives but q₂ does not. -/
theorem refinesOn_iff_questionEntails_of_partition {W : Type*}
    (q₁ q₂ : Discourse.Issue W) (worlds : List W)
    (hp₁ : q₁.isPartition worlds = true)
    (hp₂ : q₂.isPartition worlds = true)
    (hne : worlds ≠ []) :
    Discourse.refinesOn (q₁.toQUD) (q₂.toQUD) worlds =
    Discourse.questionEntails q₁ q₂ worlds := by
  rw [Bool.eq_iff_iff]
  exact ⟨forward q₁ q₂ worlds hp₁ hp₂ hne, backward q₁ q₂ worlds hp₁ hp₂⟩
