...
and s-command ([langacker-1969]).

## Main Definitions

* `commandRelation T P` — pairs `(a, b)` where every P-upper-bound of `a`
  dominates `b`.
* `branchingNodes T` / `cCommand T` — the branching-node generating
  property and [reinhart-1976]'s c-command it generates.
* `commandByRelation R` — relation-based variant generalizing the
  property-based definition.
* `mateRelation T P` — symmetric closure: `C_P ∩ (C_P)⁻¹`.
* `maximalGenerator R` — largest relation generating the same `C_R`.
...
/-- **Command relation** generated by property P ([barker-pullum-1990] Definition 3).
...
/-- **Theorem 1 (Intersection Theorem)** of [barker-pullum-1990]:
    `C_P ∩ C_Q = C_{P ∪ Q}`.
...
/-- **Mother relation** ([barker-pullum-1990] Definition 9): `a` is the
...
def mother {Node : Type} [PartialOrder Node] (T : TreeOrder Node) (a b : Node) : Prop :=
  a ∈ T.nodes ∧ b ∈ T.nodes ∧ T.properDom a b ∧
    ¬∃ x ∈ T.nodes, T.properDom a x ∧ T.properDom x b

/-- **Branching nodes** ([barker-pullum-1990] P₅, eq. (19), via the
...
def branchingNodes {Node : Type} [PartialOrder Node] (T : TreeOrder Node) : Set Node :=
  {n | ∃ a b, a ≠ b ∧ mother T n a ∧ mother T n b}

/-- **C-command** ([barker-pullum-1990] Definition 10; [reinhart-1976]
    def. 36): the command relation generated by branching nodes.
    Distinct from Lasnik's K-command ([barker-pullum-1990] Definition 7),
    which is generated by the *label-based* property "labeled S or NP"
    and is not definable on a bare `TreeOrder`. -/
def cCommand {Node : Type} [PartialOrder Node] (T : TreeOrder Node) : Set (Node × Node) :=
...
/-- A command relation `C_P` is **ambidextrous** iff for all `a`:
    ...
    [barker-pullum-1990] Theorem 3: All command relations are ambidextrous. -/
...
/-- **Embeddability with CAC** ([barker-pullum-1990] Theorem 8):
...
def commandByRelation {Node : Type} [PartialOrder Node] (R : Node → Node → Prop) :
    Set (Node × Node) :=
...
theorem command_as_relation {Node : Type} [PartialOrder Node] (T : TreeOrder Node) (P : Set Node) :
    commandRelation T P = commandByRelation (λ a x => T.properDom x a ∧ x ∈ P) := by
  ext ⟨a, b⟩
...
/-- **Command equivalence** ([barker-pullum-1990] Definition 20):
    `R ~ S` iff `C_R = C_S`. -/
def commandEquivalent {Node : Type} [PartialOrder Node] (R S : Node → Node → Prop) : Prop :=
  commandByRelation R = commandByRelation S

/-- **Maximal generator** ([barker-pullum-1990] Definition 21):
    `R̂ = ⋃ {S | S ⊇ R ∧ S ~ R}`.

    The largest relation generating the same command relation as `R`. -/
def maximalGenerator {Node : Type} [PartialOrder Node] (R : Node → Node → Prop) :
    Node → Node → Prop :=
  λ a x => ∃ S, (∀ a' x', R a' x' → S a' x') ∧
                   commandEquivalent R S ∧
                   S a x

/-- Maximal generator contains the original relation. -/
theorem maximalGenerator_contains {Node : Type} [PartialOrder Node] (R : Node → Node → Prop) :
    ∀ a x, R a x → maximalGenerator R a x := by
  intro a x hRax
  use R
...
theorem maximalGenerator_equivalent {Node : Type} [PartialOrder Node] (R : Node → Node → Prop) :
    commandEquivalent R (maximalGenerator R) := by
  unfold commandEquivalent
  ext ⟨a, b⟩
  simp only [commandByRelation, maximalGenerator, mem_ofPred_eq]
  constructor
  · intro hR x ⟨S, _hSR, hSeq, hSax⟩
    have : (a, b) ∈ commandByRelation S := by
      rw [← hSeq]
      exact hR
    exact this x hSax
  · intro h x hRax
    have hmax := maximalGenerator_contains R a x hRax
    exact h x hmax

/-- **Intersection Theorem for Relations**: `C_R ∩ C_S = C_{R ∪ S}`. -/
theorem relation_intersection_theorem {Node : Type} [PartialOrder Node]
    (R S : Node → Node → Prop) :
    commandByRelation R ∩ commandByRelation S =
      commandByRelation (λ a x => R a x ∨ S a x) := by
  ext ⟨a, b⟩
...
/-- **Union Theorem** ([barker-pullum-1990] Theorem 9, forward):
    `C_R ∪ C_S ⊆ C_{R̂ ∩ Ŝ}`. -/
theorem relation_union_theorem {Node : Type} [PartialOrder Node] (R S : Node → Node → Prop) :
    commandByRelation R ∪ commandByRelation S ⊆
      commandByRelation (λ a x => maximalGenerator R a x ∧ maximalGenerator S a x) := by
  intro ⟨a, b⟩ h x ⟨hRhat, hShat⟩
  cases h with
  | inl hCR =>
    have heq := maximalGenerator_equivalent R
    rw [heq] at hCR
    exact hCR x hRhat
  | inr hCS =>
    have heq := maximalGenerator_equivalent S
    rw [heq] at hCS
    exact hCS x hShat

/-- **Union Theorem** ([barker-pullum-1990] Theorem 9, reverse) — uses CAC. -/
theorem relation_union_theorem_reverse {Node : Type} [PartialOrder Node] (T : TreeOrder Node)
    ...
    commandByRelation (λ a x => maximalGenerator R a x ∧ maximalGenerator S a x) ⊆
  ...
  cases hCAC with
  | inl hcd =>
    have hcInRhat : maximalGenerator R a c := maximalGenerator_contains R a c hRac
    have hcInShat : maximalGenerator S a c :=
  ...
  | inr hdc =>
    have hdInShat : maximalGenerator S a d := maximalGenerator_contains S a d hSad
    have hdInRhat : maximalGenerator R a d :=
