...
Unlike `Core.Order.Bilattice.Representation` (which handles the distributive
...
product of two lattices is interlaced — is `Core.Order.Bilattice.Product`.
...
The converse of `Core.Order.Bilattice.Product`: every interlaced bilattice is
