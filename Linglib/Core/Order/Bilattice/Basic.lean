...
[fitting-1994] [schoter-1996b]

The *evidential* bilattices ([schoter-1996b]) are the diagonal Ginsberg–Fitting
...
is [schoter-1996b]'s 9-valued `PRESUP` (see `Studies.Schoter1996b`).
...
values are `(for, against)` pairs ([schoter-1996b]). -/
...
([fitting-1994]; [schoter-1996b]'s `CLAS`). -/
...
/-- Fitting's guard connective `φ : ψ` ([schoter-1996b] Def 4): the value of the
...
[schoter-1996b] Def 4 notes — though it is not a lattice-theoretic connective. -/
...
second coordinate ([schoter-1996b] Def 4). -/
