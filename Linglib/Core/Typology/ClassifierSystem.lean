...
Per-language schema for `Core.Typology.LanguageProfile`'s
`classifierSystem` field, parallel to `Core.Typology.WordOrder` and
`Core.Typology.Adposition`. Captures @cite{aikhenvald-2000}'s 7
