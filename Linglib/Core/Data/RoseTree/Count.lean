...
* `RoseTree.leafCountP` / `RoseTree.leafDepthsP` / `RoseTree.leafDepthSumP` (with
  `UnorderedTree` descents): leaf statistics by predicate over the leaf projection — the
...
* `UnorderedTree.Forest.numNodes` / `UnorderedTree.Forest.numEdges`: the forest totals.

## Main results

* `UnorderedTree.Forest.numNodes_eq_card_add_numEdges`: Euler's relation `#V = b₀ + #E`
...
`[UPSTREAM]` candidate alongside the `UnorderedTree` carrier.
-/
...
/-- The number of leaves whose label satisfies `p`. -/
def leafCountP : UnorderedTree α → ℕ :=
  UnorderedTree.lift (RoseTree.leafCountP p) fun _ _ => RoseTree.leafCountP_perm p

@[simp] theorem leafCountP_mk (t : RoseTree α) :
    (mk t).leafCountP p = t.leafCountP p := rfl

@[simp] theorem leafCountP_leaf (a : α) :
    (leaf a : UnorderedTree α).leafCountP p = if p a then 1 else 0 :=
  RoseTree.leafCountP_leaf p a

/-- A root failing `p` contributes nothing: the count is the children's total. -/
theorem leafCountP_node_of_not (a : α)
    (F : Multiset (UnorderedTree α)) (h : ¬p a) :
    (UnorderedTree.node a F).leafCountP p = (F.map (UnorderedTree.leafCountP p)).sum := by
...
/-- The sum of root-distances of the leaves whose label satisfies `p`. -/
def leafDepthSumP : UnorderedTree α → ℕ :=
  UnorderedTree.lift (RoseTree.leafDepthSumP p) fun _ _ => RoseTree.leafDepthSumP_perm p

@[simp] theorem leafDepthSumP_mk (t : RoseTree α) :
    (mk t).leafDepthSumP p = t.leafDepthSumP p := rfl

@[simp] theorem leafDepthSumP_leaf (a : α) :
    (leaf a : UnorderedTree α).leafDepthSumP p = 0 :=
...
@[simp] theorem leafDepthSumP_node (a : α)
    ...
    (UnorderedTree.node a F).leafDepthSumP p
...
theorem leafCountP_lt_numNodes_of_not_root
    (t : UnorderedTree α) (h : ¬p t.rootValue) : t.leafCountP p < t.numNodes := by
  obtain ⟨t₀, rfl⟩ : ∃ t₀ : RoseTree α, t = UnorderedTree.mk t₀ :=
    ⟨t.out, (Quotient.out_eq t).symm⟩
  rw [UnorderedTree.rootValue_mk] at h
  cases t₀ with
  | node x cs =>
    rw [RoseTree.value_node] at h
    rw [UnorderedTree.leafCountP_mk, UnorderedTree.numNodes_mk]
...
theorem leafCountP_le_leafDepthSumP_of_not_root
    (t : UnorderedTree α) (h : ¬p t.rootValue) : t.leafCountP p ≤ t.leafDepthSumP p := by
  obtain ⟨t₀, rfl⟩ : ∃ t₀ : RoseTree α, t = UnorderedTree.mk t₀ :=
    ⟨t.out, (Quotient.out_eq t).symm⟩
  rw [UnorderedTree.rootValue_mk] at h
  cases t₀ with
  | node x cs =>
    rw [RoseTree.value_node] at h
    rw [UnorderedTree.leafCountP_mk, UnorderedTree.leafDepthSumP_mk]
...
theorem leafCountP_le_numEdges (t : UnorderedTree α)
